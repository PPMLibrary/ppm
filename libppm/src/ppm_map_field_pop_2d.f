      ...
      !-------------------------------------------------------------------------
      !
      !  Purpose      : This routine pops the list buffer for 2d mesh data
      !
      !  Input        : fdata([:,]:,:,:)(F) field data. 1st index: lda,
      !                                     2nd-3th: mesh (i,j) relative
      !                                     to istart-1 of the sub (i.e.
      !                                     (i,j)=(1,1) corresponds to
      !                                     istart), 4th: isub 1...nsublist
      !                                     (all subs on this processor).
      !                                     For scalar fields, the first
      !                                     index is omitted (the others
      !                                     shift accordingly).
      ...
      !                                     (internal numbering)
      !                 ghostsize(:)    (I) size of the ghost layer (in
      !                                     mesh points) in each dimension.
      ...
      !
      !  Remarks      : The on-processor data is in the first part of the
      !                 buffer
      !
      ...
      !
      !  Revision 1.26  2004/11/11 15:23:02  ivos
      !  Moved allocatable work data to the module.
      !
      !  Revision 1.25  2004/10/01 16:09:05  ivos
      !  Replaced REAL(ppm_kind_double) :: t0 with REAL(MK) t0.
      !
      !  Revision 1.24  2004/09/22 10:43:24  ivos
      ...
      !
      !  Revision 1.12  2004/04/07 09:20:48  ivos
      ...
      !
      !  Revision 1.8  2004/04/02 07:43:21  ivos
      !  Replaced direct allocation of fdata with call to ppm_alloc to use
      !  the newly added 4d version of it.
      !
      !  Revision 1.7  2004/03/05 13:43:22  ivos
      !  bugfix: index errors corrected for COMPLEX version. COMPLEX version
      !  is now tested.
      !
      !  Revision 1.6  2004/02/24 12:16:04  ivos
      ...
      !
      !  Revision 1.5  2004/02/24 08:46:54  ivos
      !  Added overloading for single complex and double complex data types.
      !
      !  Revision 1.4  2004/02/23 12:19:00  ivos
      ...
      ! ATTN: DIM is the dimension of the fddata array and not the space
      ! dimemsion ppm_dim!
#if    __DIM == __SFIELD
#if    __KIND == __SINGLE_PRECISION
      SUBROUTINE ppm_map_field_pop_2d_sca_s(fdata,lda,tomesh,ghostsize,rtype, &
     &    info,mask)
#elif  __KIND == __DOUBLE_PRECISION
      SUBROUTINE ppm_map_field_pop_2d_sca_d(fdata,lda,tomesh,ghostsize,rtype, &
     &    info,mask)
#elif  __KIND == __SINGLE_PRECISION_COMPLEX
      SUBROUTINE ppm_map_field_pop_2d_sca_sc(fdata,lda,tomesh,ghostsize,rtype,&
     &    info,mask)
#elif  __KIND == __DOUBLE_PRECISION_COMPLEX
      SUBROUTINE ppm_map_field_pop_2d_sca_dc(fdata,lda,tomesh,ghostsize,rtype,&
     &    info,mask)
#elif  __KIND == __INTEGER
      SUBROUTINE ppm_map_field_pop_2d_sca_i(fdata,lda,tomesh,ghostsize,rtype, &
     &    info,mask)
...
#elif  __DIM == __VFIELD
#if    __KIND == __SINGLE_PRECISION
      SUBROUTINE ppm_map_field_pop_2d_vec_s(fdata,lda,tomesh,ghostsize,rtype, &
     &    info,mask)
#elif  __KIND == __DOUBLE_PRECISION
      SUBROUTINE ppm_map_field_pop_2d_vec_d(fdata,lda,tomesh,ghostsize,rtype, &
     &    info,mask)
#elif  __KIND == __SINGLE_PRECISION_COMPLEX
      SUBROUTINE ppm_map_field_pop_2d_vec_sc(fdata,lda,tomesh,ghostsize,rtype,&
     &    info,mask)
#elif  __KIND == __DOUBLE_PRECISION_COMPLEX
      SUBROUTINE ppm_map_field_pop_2d_vec_dc(fdata,lda,tomesh,ghostsize,rtype,&
     &    info,mask)
#elif  __KIND == __INTEGER
      SUBROUTINE ppm_map_field_pop_2d_vec_i(fdata,lda,tomesh,ghostsize,rtype, &
     &    info,mask)
#elif  __KIND == __LOGICAL
...
#include "ppm_define.h"
     ...
     &                'y dimension of mask does not match mesh',__LINE__,info)
                  GOTO 9999
              ENDIF
              IF (SIZE(mask,3) .LT. ppm_nsublist(totopo)) THEN
                  info = ppm_error_error
                  CALL ppm_error(ppm_err_argument,'ppm_map_field_pop_2d',  &
