      ...
      !-------------------------------------------------------------------------
      !
      !  Purpose      : This routine receives/updates the values of the
      !                 ghost mesh points of each sub from its
      !                 corresponding neighbor subs (i.e. adds ghosts
      !                 layers to the subs of the current topology).
      !                 ppm_map_field_ghost_init must be called before this
      !                 in order to build all the lists needed.
      !
      ...
      !  Revisions    :
      !-------------------------------------------------------------------------
      !  $Log: ppm_map_field_ghost_get.f,v $
      !  Revision 1.1.1.1  2007/07/13 10:18:55  ivos
      !  CBL version of the PPM library
      !
      !  Revision 1.7  2004/10/01 16:33:35  ivos
      !  cosmetics.
      !
      !  Revision 1.6  2004/10/01 16:09:04  ivos
      !  Replaced REAL(ppm_kind_double) :: t0 with REAL(MK) t0.
      !
      !  Revision 1.5  2004/08/31 12:48:08  ivos
      !  Changed argument checks to use ppm_check_topoid and ppm_check_meshid.
      !
      !  Revision 1.4  2004/07/26 11:46:55  ivos
      ...
      SUBROUTINE ppm_map_field_ghost_get(topoid,meshid,info)
...
#include "ppm_define.h"
     ...
     &            'meshid out of range',__LINE__,info)
      ...
      !-------------------------------------------------------------------------
      nsendlist = ppm_mesh_ghost_nsend(meshid,topoid)
      nrecvlist = ppm_mesh_ghost_nrecv(meshid,topoid)
     ...
     &        'global recv buffer pointer PPM_PRECVBUFFER',__LINE__,info)
      ...
      DO i=1,ppm_ncommseq(topoid)
         ...
         !----------------------------------------------------------------------
         ppm_psendbuffer(ibuffer)=ppm_mesh_ghost_blk(ibuffer,meshid,topoid)
         ppm_precvbuffer(ibuffer)=ppm_mesh_ghost_recvblk(ibuffer,meshid,topoid)
     ...
     &           ppm_mesh_ghost_blksize(1:pdim,j,meshid,topoid)
             ppm_mesh_isendfromsub(j) = ppm_mesh_ghost_fromsub(j,meshid,topoid)
         ENDDO
     ...
     &           ppm_mesh_ghost_recvblksize(1:pdim,j,meshid,topoid)
             ppm_mesh_irecvtosub(j) = ppm_mesh_ghost_recvtosub(j,meshid,topoid)
         ENDDO
      ENDDO
