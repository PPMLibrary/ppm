      ...
      SUBROUTINE ppm_map_field_push_2d_sca_s(topoid,meshid,fdata,info,mask)
#elif  __KIND == __DOUBLE_PRECISION
      SUBROUTINE ppm_map_field_push_2d_sca_d(topoid,meshid,fdata,info,mask)
#elif  __KIND == __SINGLE_PRECISION_COMPLEX
      SUBROUTINE ppm_map_field_push_2d_sca_sc(topoid,meshid,fdata,info,mask)
#elif  __KIND == __DOUBLE_PRECISION_COMPLEX
      SUBROUTINE ppm_map_field_push_2d_sca_dc(topoid,meshid,fdata,info,mask)
#elif  __KIND == __INTEGER
      SUBROUTINE ppm_map_field_push_2d_sca_i(topoid,meshid,fdata,info,mask)
#elif  __KIND == __LOGICAL
      SUBROUTINE ppm_map_field_push_2d_sca_l(topoid,meshid,fdata,info,mask)
#endif
#elif  __DIM == __VFIELD
#if    __KIND == __SINGLE_PRECISION
      SUBROUTINE ppm_map_field_push_2d_vec_s(topoid,meshid,fdata,lda,info,mask)
#elif  __KIND == __DOUBLE_PRECISION
      SUBROUTINE ppm_map_field_push_2d_vec_d(topoid,meshid,fdata,lda,info,mask)
#elif  __KIND == __SINGLE_PRECISION_COMPLEX
      SUBROUTINE ppm_map_field_push_2d_vec_sc(topoid,meshid,fdata,lda,info,mask)
#elif  __KIND == __DOUBLE_PRECISION_COMPLEX
      SUBROUTINE ppm_map_field_push_2d_vec_dc(topoid,meshid,fdata,lda,info,mask)
#elif  __KIND == __INTEGER
      SUBROUTINE ppm_map_field_push_2d_vec_i(topoid,meshid,fdata,lda,info,mask)
#elif  __KIND == __LOGICAL
      SUBROUTINE ppm_map_field_push_2d_vec_l(topoid,meshid,fdata,lda,info,mask)
...
#include "ppm_define.h"
 ...
 9999 CONTINUE
     ...
     &            'lda must be =1 for scalar data',__LINE__,info)
              GOTO 8888
          ENDIF
          IF (SIZE(fdata,3) .LT. ppm_topo(topoid)%t%nsublist) THEN
              info = ppm_error_error
              CALL ppm_error(ppm_err_argument,'ppm_map_field_push_2d',  &
     &            'field data for some subs is missing',__LINE__,info)
              GOTO 8888
     ...
     &                'y dimension of mask does not match mesh',__LINE__,info)
                  GOTO 8888
              ENDIF
              IF (SIZE(mask,3) .LT. ppm_topo(topoid)%t%nsublist) THEN
                  info = ppm_error_error
                  CALL ppm_error(ppm_err_argument,'ppm_map_field_push_2d',  &
