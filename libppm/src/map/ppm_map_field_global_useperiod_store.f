      ...
      SUBROUTINE ppm_map_field_global_useperiod_store(topoid,target_topoid,  &
     &                                meshid,target_meshid,info)
      ...
      !!! with all mesh blocks that have to be send and/or received are
      !!! built in this routine. Push, pop and send will use these lists.
      !!!
      !!! [WARNING]
...
#include "ppm_define.h"
      ...
      !  Initialise
      !-------------------------------------------------------------------------
      CALL substart('ppm_map_field_global_useperiod_store',t0,info)
      pdim = ppm_dim
      ...
      IF (ppm_buffer_set .GT. 0) THEN
        ...
        CALL ppm_error(ppm_err_map_incomp,'ppm_map_field_global_usep_store', &
     ...
     &            'source and destination meshes are incompatible',__LINE__,info)
      ...
      !-------------------------------------------------------------------------
      !  Check if this mapping has already been done and stored
      !-------------------------------------------------------------------------

      IF (.NOT.ASSOCIATED(mesh%mapping)) THEN
!          iopt   = ppm_param_alloc_fit
!          ldu(1) = ppm_max_topoid ! FIXME
!          CALL ppm_mesh_alloc(ppm_maplist,ldu,iopt,info)
!          IF (info .NE. 0) THEN
!             info = ppm_error_fatal
!             CALL ppm_error(ppm_err_alloc,'ppm_map_field_global',  &
!     &           'list of stored mappings PPM_MAP_FIELD_LIST',__LINE__,info)
!             GOTO 9999
!          ENDIF
      ...
      IF (info .NE. 0) THEN
          info = ppm_error_fatal
          CALL ppm_error(ppm_err_alloc,'ppm_map_field_global_usep_store', &
     ...
     &                ioffset,info,lsymm)
      ...
      IF (info .NE. 0) THEN
          info = ppm_error_fatal
          CALL ppm_error(ppm_err_alloc,'ppm_map_field_globa_usep_storel', &
     ...
     &                   ' to ',sendrank
                     ENDIF
                     CALL ppm_write(ppm_rank,'ppm_map_field_global_usep_store',&
     &                              mesg,info)
                 ENDIF
             ENDIF
         ENDDO
     ...
     &                              mesg,info)
      ...
      IF (info .NE. 0) THEN
          info = ppm_error_error
          CALL ppm_error(ppm_err_dealloc,'ppm_map_field_global_usep_store', &
     &        'local send source sub list ISENDFROMSUB',__LINE__,info)
      ENDIF
      CALL ppm_alloc(isendtosub,ldu,iopt,info)
      IF (info .NE. 0) THEN
     ...
     &        'local send block size list ISENDBLKSIZE',__LINE__,info)
      ...
      IF (info .NE. 0) THEN
          info = ppm_error_error
          CALL ppm_error(ppm_err_dealloc,'ppm_map_field_globa_usep_store', &
     &        'local recv source sub list IRECVFROMSUB',__LINE__,info)
      ENDIF
      CALL ppm_alloc(irecvtosub,ldu,iopt,info)
      IF (info .NE. 0) THEN
     ...
     &        'local recv block offset list IOFFSET',__LINE__,info)
      ...
      !-------------------------------------------------------------------------
 9999 CONTINUE
      CALL substop('ppm_map_field_global_useperiod_store',t0,info)
      RETURN
      ...
      SUBROUTINE check
          ...
          IF (.NOT. valid) THEN
              info = ppm_error_error
              CALL ppm_error(ppm_err_argument,'ppm_map_field_global_usep_store',&
     &            'target topoid not valid',__LINE__,info)
          ...
          IF (.NOT. valid) THEN
              info = ppm_error_error
              CALL ppm_error(ppm_err_argument,'ppm_map_field_globa_usep_storel',&
 ...
 8888     CONTINUE
      ...
      END SUBROUTINE ppm_map_field_global_useperiod_store
