...
SUBROUTINE __FUNCNAME(Particles,wp,ppt_id,with_ghosts)
    ...
    IF (ppt_id .LE. Particles%max_wpid) THEN
        IF (Particles%props(ppt_id)%t%flags(ppm_ppt_partial)) THEN
            ...
            wp => &
#if   __DIM == 1
                Particles%props(ppt_id)%t%WRAP(DATANAME)(1:Particles%Npart)
#elif __DIM == 2
                Particles%props(ppt_id)%t%WRAP(DATANAME)(:,1:Particles%Npart)
#endif
            RETURN
...
SUBROUTINE __FUNCNAME(Particles,wp,ppt_id,read_only,ghosts_ok)
    ...
    Particles%props(ppt_id)%t%flags(ppm_ppt_ghosts) = .FALSE.
    wp => NULL()
