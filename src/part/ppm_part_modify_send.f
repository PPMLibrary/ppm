      !-------------------------------------------------------------------------
      !  Subroutine   :                 ppm_part_modify_send
      !-------------------------------------------------------------------------
      ...
      !  Initialise 
      !-------------------------------------------------------------------------
      CALL substart('ppm_part_modify_send',t0,info)
      !-------------------------------------------------------------------------
      !  Check arguments
      !-------------------------------------------------------------------------
      ...
      IF (ppm_buffer_set .LT. 1) THEN
        ...
        IF (ppm_debug .GT. 1) THEN
            CALL ppm_error(ppm_err_buffer_empt,'ppm_part_modify_send',    &
     ...
     &        'send buffer PPM_SENDBUFFER',__LINE__,info)
      ...
      IF (info .NE. 0) THEN
          info = ppm_error_error
          CALL ppm_error(ppm_err_dealloc,'ppm_part_modify_send',     &
     ...
     &        'local send buffer SENDS',__LINE__,info)
      ...
      !-------------------------------------------------------------------------
 9999 CONTINUE
      CALL substop('ppm_part_modify_send',t0,info)
      RETURN
      CONTAINS
      SUBROUTINE check
 8888   CONTINUE
