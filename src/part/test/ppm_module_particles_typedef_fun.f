! ppm_module_particles_typedef_fun.f - a unit test suite for ppm_module_particles_typedef.f
!
! funit generated this file from ppm_module_particles_typedef.fun

module ppm_module_particles_typedef_fun
...
use ppm_module_data
 ...
 public :: test_ppm_module_particles_typedef
...
logical, dimension(:),   pointer               :: wp_1l => NULL()
 ...
 subroutine initialize_cart
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test initialize_cart &
                 [ppm_module_particles_typedef.fun:134] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test initialize_cart &
              &[ppm_module_particles_typedef.fun:134]"
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test initialize_cart &
                 [ppm_module_particles_typedef.fun:138] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test initialize_cart &
              &[ppm_module_particles_typedef.fun:138]"
      write(log,*) "  ", "info (",info,") is not", 0
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test initialize_cart &
                 [ppm_module_particles_typedef.fun:141] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test initialize_cart &
              &[ppm_module_particles_typedef.fun:141]"
      write(log,*) "  ", "info (",info,") is not", 0
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test initialize_cart &
                 [ppm_module_particles_typedef.fun:146] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test initialize_cart &
              &[ppm_module_particles_typedef.fun:146]"
      write(log,*) "  ", "info (",info,") is not", 0
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test initialize_cart &
                 [ppm_module_particles_typedef.fun:152] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test initialize_cart &
              &[ppm_module_particles_typedef.fun:152]"
      write(log,*) "  ", "info (",info,") is not", 0
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test initialize_cart &
                 [ppm_module_particles_typedef.fun:158] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test initialize_cart &
              &[ppm_module_particles_typedef.fun:158]"
      write(log,*) "  ", "info (",info,") is not", 0
  ...
  numAsserts = numAsserts + 1
  if (noAssertFailed) then
    write(log,'(A)', advance='no') "trying assert         Assert_Equal(MINVAL(wp_2i),0)"
    if (.not.(MINVAL(wp_2i)==0)) then
      write(log,*) " failed!"
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test initialize_cart &
                 [ppm_module_particles_typedef.fun:160] "
      write(*,*) "MINVAL(wp_2i) (",MINVAL(wp_2i),") is not", 0
      write(log,*) " *Assert_Equal failed* in test initialize_cart &
              &[ppm_module_particles_typedef.fun:160]"
      write(log,*) "  ", "MINVAL(wp_2i) (",MINVAL(wp_2i),") is not", 0
      write(log,*) ""
      noAssertFailed = .false.
      numFailures    = numFailures + 1
#ifdef __MPI
  ...
  numAsserts = numAsserts + 1
  if (noAssertFailed) then
    write(log,'(A)', advance='no') "trying assert         Assert_Equal(MAXVAL(wp_2i),0)"
    if (.not.(MAXVAL(wp_2i)==0)) then
      write(log,*) " failed!"
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test initialize_cart &
                 [ppm_module_particles_typedef.fun:161] "
      write(*,*) "MAXVAL(wp_2i) (",MAXVAL(wp_2i),") is not", 0
      write(log,*) " *Assert_Equal failed* in test initialize_cart &
              &[ppm_module_particles_typedef.fun:161]"
      write(log,*) "  ", "MAXVAL(wp_2i) (",MAXVAL(wp_2i),") is not", 0
      write(log,*) ""
      noAssertFailed = .false.
      numFailures    = numFailures + 1
#ifdef __MPI
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test initialize_cart &
                 [ppm_module_particles_typedef.fun:167] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test initialize_cart &
              &[ppm_module_particles_typedef.fun:167]"
      write(log,*) "  ", "info (",info,") is not", 0
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test initialize_cart &
                 [ppm_module_particles_typedef.fun:172] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test initialize_cart &
              &[ppm_module_particles_typedef.fun:172]"
      write(log,*) "  ", "info (",info,") is not", 0
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test initialize_cart &
                 [ppm_module_particles_typedef.fun:179] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test initialize_cart &
              &[ppm_module_particles_typedef.fun:179]"
      write(log,*) "  ", "info (",info,") is not", 0
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test initialize_cart &
                 [ppm_module_particles_typedef.fun:181] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test initialize_cart &
              &[ppm_module_particles_typedef.fun:181]"
      write(log,*) "  ", "info (",info,") is not", 0
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test initialize_cart &
                 [ppm_module_particles_typedef.fun:186] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test initialize_cart &
              &[ppm_module_particles_typedef.fun:186]"
      write(log,*) "  ", "info (",info,") is not", 0
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test initialize_cart &
                 [ppm_module_particles_typedef.fun:188] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test initialize_cart &
              &[ppm_module_particles_typedef.fun:188]"
      write(log,*) "  ", "info (",info,") is not", 0
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test initialize_cart &
                 [ppm_module_particles_typedef.fun:193] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test initialize_cart &
              &[ppm_module_particles_typedef.fun:193]"
      write(log,*) "  ", "info (",info,") is not", 0
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test initialize_cart &
                 [ppm_module_particles_typedef.fun:195] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test initialize_cart &
              &[ppm_module_particles_typedef.fun:195]"
      write(log,*) "  ", "info (",info,") is not", 0
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test initialize_cart &
                 [ppm_module_particles_typedef.fun:209] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test initialize_cart &
              &[ppm_module_particles_typedef.fun:209]"
      write(log,*) "  ", "info (",info,") is not", 0
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test initialize_cart &
                 [ppm_module_particles_typedef.fun:215] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test initialize_cart &
              &[ppm_module_particles_typedef.fun:215]"
      write(log,*) "  ", "info (",info,") is not", 0
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test initialize_cart &
                 [ppm_module_particles_typedef.fun:217] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test initialize_cart &
              &[ppm_module_particles_typedef.fun:217]"
      write(log,*) "  ", "info (",info,") is not", 0
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test initialize_cart &
                 [ppm_module_particles_typedef.fun:221] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test initialize_cart &
              &[ppm_module_particles_typedef.fun:221]"
      write(log,*) "  ", "info (",info,") is not", 0
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test initialize_cart &
                 [ppm_module_particles_typedef.fun:233] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test initialize_cart &
              &[ppm_module_particles_typedef.fun:233]"
      write(log,*) "  ", "info (",info,") is not", 0
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test initialize_cart &
                 [ppm_module_particles_typedef.fun:235] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test initialize_cart &
              &[ppm_module_particles_typedef.fun:235]"
      write(log,*) "  ", "info (",info,") is not", 0
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test initialize_cart &
                 [ppm_module_particles_typedef.fun:238] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test initialize_cart &
              &[ppm_module_particles_typedef.fun:238]"
      write(log,*) "  ", "info (",info,") is not", 0
  ...
  numAsserts = numAsserts + 1
  if (noAssertFailed) then
    write(log,'(A)', advance='no') "trying assert         Assert_Equal_Within(err,0,tol)"
    if (.not.((0 &
     +tol) &
     .ge. &
     (err) &
     ...
     (err) )) then
      write(log,*) " failed!"
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal_Within failed* in test initialize_cart &
                 [ppm_module_particles_typedef.fun:247] "
      write(*,*) "err (",err,") is not", &
 0,"within",tol
      write(log,*) " *Assert_Equal_Within failed* in test initialize_cart &
              &[ppm_module_particles_typedef.fun:247]"
      write(log,*) "  ", "err (",err,") is not", &
 0,"within",tol
      write(log,*) ""
      noAssertFailed = .false.
      numFailures    = numFailures + 1
 ...
 end subroutine initialize_cart


 subroutine neighlists
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      write(log,*) " failed!"
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test neighlists &
                 [ppm_module_particles_typedef.fun:263] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test neighlists &
              &[ppm_module_particles_typedef.fun:263]"
      write(log,*) "  ", "info (",info,") is not", 0
      write(log,*) ""
      noAssertFailed = .false.
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test neighlists &
                 [ppm_module_particles_typedef.fun:269] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test neighlists &
              &[ppm_module_particles_typedef.fun:269]"
      write(log,*) "  ", "info (",info,") is not", 0
 ...
 subroutine sop_type
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      write(log,*) " failed!"
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test sop_type &
                 [ppm_module_particles_typedef.fun:283] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test sop_type &
              &[ppm_module_particles_typedef.fun:283]"
      write(log,*) "  ", "info (",info,") is not", 0
      write(log,*) ""
      noAssertFailed = .false.
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test sop_type &
                 [ppm_module_particles_typedef.fun:287] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test sop_type &
              &[ppm_module_particles_typedef.fun:287]"
      write(log,*) "  ", "info (",info,") is not", 0
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test sop_type &
                 [ppm_module_particles_typedef.fun:290] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test sop_type &
              &[ppm_module_particles_typedef.fun:290]"
      write(log,*) "  ", "info (",info,") is not", 0
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test sop_type &
                 [ppm_module_particles_typedef.fun:293] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test sop_type &
              &[ppm_module_particles_typedef.fun:293]"
      write(log,*) "  ", "info (",info,") is not", 0
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test sop_type &
                 [ppm_module_particles_typedef.fun:301] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test sop_type &
              &[ppm_module_particles_typedef.fun:301]"
      write(log,*) "  ", "info (",info,") is not", 0
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test sop_type &
                 [ppm_module_particles_typedef.fun:307] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test sop_type &
              &[ppm_module_particles_typedef.fun:307]"
      write(log,*) "  ", "info (",info,") is not", 0
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test sop_type &
                 [ppm_module_particles_typedef.fun:313] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test sop_type &
              &[ppm_module_particles_typedef.fun:313]"
      write(log,*) "  ", "info (",info,") is not", 0
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test sop_type &
                 [ppm_module_particles_typedef.fun:315] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test sop_type &
              &[ppm_module_particles_typedef.fun:315]"
      write(log,*) "  ", "info (",info,") is not", 0
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test sop_type &
                 [ppm_module_particles_typedef.fun:329] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test sop_type &
              &[ppm_module_particles_typedef.fun:329]"
      write(log,*) "  ", "info (",info,") is not", 0
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test sop_type &
                 [ppm_module_particles_typedef.fun:335] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test sop_type &
              &[ppm_module_particles_typedef.fun:335]"
      write(log,*) "  ", "info (",info,") is not", 0
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test sop_type &
                 [ppm_module_particles_typedef.fun:337] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test sop_type &
              &[ppm_module_particles_typedef.fun:337]"
      write(log,*) "  ", "info (",info,") is not", 0
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test sop_type &
                 [ppm_module_particles_typedef.fun:341] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test sop_type &
              &[ppm_module_particles_typedef.fun:341]"
      write(log,*) "  ", "info (",info,") is not", 0
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test sop_type &
                 [ppm_module_particles_typedef.fun:353] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test sop_type &
              &[ppm_module_particles_typedef.fun:353]"
      write(log,*) "  ", "info (",info,") is not", 0
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test sop_type &
                 [ppm_module_particles_typedef.fun:355] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test sop_type &
              &[ppm_module_particles_typedef.fun:355]"
      write(log,*) "  ", "info (",info,") is not", 0
  ...
  if (noAssertFailed) then
    ...
    if (.not.(info==0)) then
      ...
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal failed* in test sop_type &
                 [ppm_module_particles_typedef.fun:358] "
      write(*,*) "info (",info,") is not", 0
      write(log,*) " *Assert_Equal failed* in test sop_type &
              &[ppm_module_particles_typedef.fun:358]"
      write(log,*) "  ", "info (",info,") is not", 0
  ...
  if (noAssertFailed) then
    ...
    if (.not.((0 &
     ...
     (err) )) then
      write(log,*) " failed!"
      write(*,'(A,I0,A)', advance='no') "[", funit_rank, "] "
      write(*,'(A)', advance='no') "*Assert_Equal_Within failed* in test sop_type &
                 [ppm_module_particles_typedef.fun:367] "
      write(*,*) "err (",err,") is not", &
 0,"within",tol
      write(log,*) " *Assert_Equal_Within failed* in test sop_type &
              &[ppm_module_particles_typedef.fun:367]"
      write(log,*) "  ", "err (",err,") is not", &
 0,"within",tol
      write(log,*) ""
...
pure function f0_test(pos,ndim)
 ...
 subroutine test_ppm_module_particles_typedef( nTests, nAsserts, nAssertsTested, nFailures, lfh, rank, comm )
  ...
  call funit_setup
  write(log,*) 'Entering initialize_cart...'
  call initialize_cart
  write(log,*) 'Leaving initialize_cart...'
  call funit_teardown
  write(log,*) 'cleaned up...'

  write(log,*) 'setting up...'
  call funit_setup
  write(log,*) 'Entering neighlists...'
  call neighlists
  write(log,*) 'Leaving neighlists...'
  call funit_teardown
  write(log,*) 'cleaned up...'

  write(log,*) 'setting up...'
  call funit_setup
  write(log,*) 'Entering sop_type...'
  call sop_type
  write(log,*) 'Leaving sop_type...'
  call funit_teardown
  write(log,*) 'cleaned up...'
 ...
 end subroutine test_ppm_module_particles_typedef

end module ppm_module_particles_typedef_fun
