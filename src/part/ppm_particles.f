FUNCTION DTYPE(get_xp)(Particles,with_ghosts)
    DEFINE_MK()
    TYPE(DTYPE(ppm_t_particles)),POINTER  :: Particles
    LOGICAL,OPTIONAL                      :: with_ghosts
    REAL(MK),DIMENSION(:,:),     POINTER  :: DTYPE(get_xp)
    INTEGER                               :: info

    IF (PRESENT(with_ghosts)) THEN
        IF (with_ghosts) THEN
            IF (Particles%has_ghosts) THEN
                DTYPE(get_xp) => Particles%xp(1:ppm_dim,1:Particles%Mpart)
            ELSE
    ...
    DTYPE(get_xp) => Particles%xp(1:ppm_dim,1:Particles%Npart)

END FUNCTION DTYPE(get_xp)

FUNCTION DTYPE(set_xp)(Particles,read_only,ghosts_ok)
    DEFINE_MK()
    TYPE(DTYPE(ppm_t_particles)),POINTER    :: Particles
    LOGICAL,OPTIONAL                 :: read_only
    LOGICAL,OPTIONAL                 :: ghosts_ok
    REAL(MK),DIMENSION(:,:),POINTER  :: DTYPE(set_xp)
    INTEGER                          :: info
...
END FUNCTION DTYPE(set_xp)

FUNCTION DTYPE(get_wpi)(Particles,wpi_id,with_ghosts)
    TYPE(DTYPE(ppm_t_particles)),POINTER    :: Particles
    INTEGER                                 :: wpi_id
    LOGICAL,OPTIONAL                        :: with_ghosts
    INTEGER,DIMENSION(:),POINTER            :: DTYPE(get_wpi)
    INTEGER                                 :: info

    IF (wpi_id .LE. 0) THEN
        write(cbuf,*) 'ERROR: failed to get wpi for property ',& 
            'wpi_id = ',wpi_id
        CALL ppm_write(ppm_rank,'get_wpi',cbuf,info)
        DTYPE(get_wpi) => NULL()
        RETURN
    ENDIF
            ...
            IF (PRESENT(with_ghosts)) THEN
                IF (with_ghosts) THEN
                    IF (Particles%wpi(wpi_id)%has_ghosts) THEN
                        DTYPE(get_wpi) => Particles%wpi(wpi_id)%vec(1:Particles%Mpart)
                    ELSE
                        write(*,*) line_of_stars
                        ...
                        write(*,*) DTYPE(get_wpi)(1)
#endif
                    ENDIF
                    RETURN
                ENDIF
            ENDIF
            DTYPE(get_wpi) => Particles%wpi(wpi_id)%vec(1:Particles%Npart)
            RETURN
        ENDIF
    ENDIF
    write(*,*) line_of_stars
...
END FUNCTION DTYPE(get_wpi)

FUNCTION DTYPE(set_wpi)(Particles,wpi_id,read_only,ghosts_ok)
    TYPE(DTYPE(ppm_t_particles)),POINTER    :: Particles
    INTEGER                          :: wpi_id
    LOGICAL,OPTIONAL                 :: read_only
    LOGICAL,OPTIONAL                 :: ghosts_ok
    INTEGER,DIMENSION(:),POINTER     :: DTYPE(set_wpi)
...
END FUNCTION DTYPE(set_wpi)

FUNCTION DTYPE(get_wps)(Particles,wps_id,with_ghosts)
    TYPE(DTYPE(ppm_t_particles)),POINTER    :: Particles
    DEFINE_MK()
    INTEGER                          :: wps_id
    LOGICAL,OPTIONAL                 :: with_ghosts
    REAL(MK),DIMENSION(:),POINTER  :: DTYPE(get_wps)
            ...
            IF (PRESENT(with_ghosts)) THEN
                IF (with_ghosts) THEN
                    IF (Particles%wps(wps_id)%has_ghosts) THEN
                        DTYPE(get_wps) => Particles%wps(wps_id)%vec(1:Particles%Mpart)
                    ELSE
                        write(*,*) line_of_stars
                        ...
                        !segfault the program. Compile with appropriate compiler
                        !options to check for array bounds and provide traceback
                        write(*,*) DTYPE(get_wps)(1)
#endif
                    ENDIF
                    RETURN
                ENDIF
            ENDIF
            DTYPE(get_wps) => Particles%wps(wps_id)%vec(1:Particles%Npart)
            RETURN
        ENDIF
    ENDIF
    write(*,*) line_of_stars
...
END FUNCTION DTYPE(get_wps)

FUNCTION DTYPE(set_wps)(Particles,wps_id,read_only,ghosts_ok)
    TYPE(DTYPE(ppm_t_particles)),POINTER    :: Particles
    DEFINE_MK()
    INTEGER                          :: wps_id
    LOGICAL,OPTIONAL                 :: read_only
    LOGICAL,OPTIONAL                 :: ghosts_ok
    REAL(MK),DIMENSION(:),POINTER  :: DTYPE(set_wps)
...
END FUNCTION DTYPE(set_wps)

FUNCTION DTYPE(get_wpv)(Particles,wpv_id,with_ghosts)
    TYPE(DTYPE(ppm_t_particles)),POINTER    :: Particles
    DEFINE_MK()
    INTEGER                          :: wpv_id,lda
    LOGICAL,OPTIONAL                 :: with_ghosts
    REAL(MK),DIMENSION(:,:),POINTER:: DTYPE(get_wpv)
            ...
            IF (PRESENT(with_ghosts)) THEN
                IF (with_ghosts) THEN
                    ...
                    ELSE
                        ...
                        !segfault the program. Compile with appropriate compiler
                        !options to check for array bounds and provide traceback
                        write(*,*) DTYPE(get_wpv)(1,1)
#endif
                    ENDIF
                    RETURN
                ENDIF
            ENDIF
            DTYPE(get_wpv) => Particles%wpv(wpv_id)%vec(1:lda,1:Particles%Npart)
            RETURN
        ENDIF
    ENDIF
    write(*,*) line_of_stars
...
END FUNCTION DTYPE(get_wpv)

FUNCTION DTYPE(set_wpv)(Particles,wpv_id,read_only,ghosts_ok)
    TYPE(DTYPE(ppm_t_particles)),POINTER    :: Particles
    DEFINE_MK()
    INTEGER                          :: wpv_id
    LOGICAL,OPTIONAL                 :: read_only
    LOGICAL,OPTIONAL                 :: ghosts_ok
    REAL(MK),DIMENSION(:,:),POINTER:: DTYPE(set_wpv)
...
END FUNCTION DTYPE(set_wpv)

FUNCTION DTYPE(get_dcop)(Particles,eta_id,with_ghosts)
    TYPE(DTYPE(ppm_t_particles)),POINTER    :: Particles
    DEFINE_MK()
    INTEGER                            :: eta_id
    ...
    IF (PRESENT(with_ghosts)) THEN
        IF (with_ghosts) THEN
            DTYPE(get_dcop) => Particles%ops%ker(eta_id)%vec(:,1:Particles%Mpart)
            RETURN
        ENDIF
    ENDIF
    DTYPE(get_dcop) => Particles%ops%ker(eta_id)%vec(:,1:Particles%Npart)

END FUNCTION DTYPE(get_dcop)

FUNCTION DTYPE(set_dcop)(Particles,eta_id)
    TYPE(DTYPE(ppm_t_particles)),POINTER    :: Particles
    DEFINE_MK()
    INTEGER                           :: eta_id
...
SUBROUTINE DTYPE(particles_dcop_deallocate)(Particles,info)
...
END SUBROUTINE DTYPE(particles_dcop_deallocate)


SUBROUTINE DTYPE(particles_allocate_wpi)(Particles,wp_id,info,with_ghosts,&
        zero,iopt,name)
...
END SUBROUTINE DTYPE(particles_allocate_wpi)

SUBROUTINE DTYPE(particles_allocate_wps)(Particles,wp_id,info,with_ghosts,&
        zero,iopt,name)
...
END SUBROUTINE DTYPE(particles_allocate_wps)


SUBROUTINE DTYPE(particles_allocate_wpv)(Particles,wp_id,lda,info, & 
...
END SUBROUTINE DTYPE(particles_allocate_wpv)

SUBROUTINE DTYPE(particles_mapping_global)(Particles,topoid,info,debug)
...
END SUBROUTINE DTYPE(particles_mapping_global)

SUBROUTINE DTYPE(particles_mapping_partial)(Particles,topoid,info,debug)
...
END SUBROUTINE DTYPE(particles_mapping_partial)

SUBROUTINE DTYPE(particles_mapping_ghosts)(Particles,topoid,info,ghostsize,debug)
...
END SUBROUTINE DTYPE(particles_mapping_ghosts)

SUBROUTINE DTYPE(particles_apply_bc)(Particles,topoid,info)
...
END SUBROUTINE DTYPE(particles_apply_bc)

SUBROUTINE DTYPE(particles_move)(Particles,disp,info)
...
END SUBROUTINE DTYPE(particles_move)

SUBROUTINE DTYPE(particles_have_moved)(Particles,info)
    !-----------------------------------------------------------------
...
END SUBROUTINE DTYPE(particles_have_moved)

SUBROUTINE DTYPE(particles_neighlists)(Particles,topoid,info,&
            ...
            ELSE
                conventionalinl: IF (Particles%conventionalinl) THEN
                    ...
                    CALL cnl_vlist(Particles%xp,&
                        ...
                        ppm_topo(topoid)%t%min_subs(:,1)-Particles%cutoff,&
                        ppm_topo(topoid)%t%max_subs(:,1)+Particles%cutoff,&
...
END SUBROUTINE DTYPE(particles_neighlists)

SUBROUTINE DTYPE(particles_neighlists_xset)(Particles_1,Particles_2,topoid,info,&
        lstore,knn)
    !------------------------------------------------------------------------
    ...
    !  Arguments
    !-------------------------------------------------------------------------
    TYPE(DTYPE(ppm_t_particles)),POINTER, INTENT(INOUT)    :: Particles_1
    !!! Data structure containing the particles
    TYPE(DTYPE(ppm_t_particles)),POINTER, INTENT(IN   )    :: Particles_2
    !!! Data structure containing the particles
    DEFINE_MK()
    !!! Data structure containing the particles
    INTEGER,                            INTENT(IN   )      :: topoid
...
END SUBROUTINE DTYPE(particles_neighlists_xset)

SUBROUTINE DTYPE(particles_updated_positions)(Particles,info)
    !-----------------------------------------------------------------
...
END SUBROUTINE DTYPE(particles_updated_positions)

SUBROUTINE DTYPE(particles_updated_nb_part)(Particles,info,&
...
END SUBROUTINE DTYPE(particles_updated_nb_part)

SUBROUTINE DTYPE(particles_update_cutoff)(Particles,cutoff,info)
...
END SUBROUTINE DTYPE(particles_update_cutoff)

SUBROUTINE DTYPE(particles_updated_cutoff)(Particles,info,max_cutoff_new)
...
END SUBROUTINE DTYPE(particles_updated_cutoff)

SUBROUTINE DTYPE(particles_compute_hmin)(Particles,info)
    !-----------------------------------------------------------------
...
END SUBROUTINE DTYPE(particles_compute_hmin)
...
SUBROUTINE DTYPE(particles_initialize)(Particles,Npart_global,info,&
    ...
    ELSE
        CALL  particles_initialize3d(Particles,Npart_global,info,&
            distrib,topoid,minphys,maxphys,cutoff,name=name)
    ENDIF
END SUBROUTINE DTYPE(particles_initialize)


SUBROUTINE DTYPE(particles_io_xyz)(Particles,itnum,writedir,info,&
...
END SUBROUTINE DTYPE(particles_io_xyz)

SUBROUTINE DTYPE(particles_dcop_define)(Particles,eta_id,coeffs,degree,order,nterms,&
...
END SUBROUTINE DTYPE(particles_dcop_define)

SUBROUTINE DTYPE(particles_dcop_free)(Particles,eta_id,info)
    !!!------------------------------------------------------------------------!
...
END SUBROUTINE DTYPE(particles_dcop_free)

SUBROUTINE DTYPE(particles_dcop_apply)(Particles,from_id,to_id,eta_id,&
...
END SUBROUTINE DTYPE(particles_dcop_apply)

SUBROUTINE DTYPE(particles_apply_dcops)(Particles,from_id,to_id,eta_id,sig,&
    ...
    ! Optional argument
    !-------------------------------------------------------------------------
    TYPE(DTYPE(ppm_t_particles)),POINTER, OPTIONAL,INTENT(IN) :: Particles_old
...
END SUBROUTINE DTYPE(particles_apply_dcops)

SUBROUTINE DTYPE(particles_print_stats)(Particles,info)
    !!!------------------------------------------------------------------------!
    ...
    REAL(KIND(1.D0))                           :: t0
    TYPE(DTYPE(particles_stats)),POINTER       :: stats
...
END SUBROUTINE DTYPE(particles_print_stats)

SUBROUTINE DTYPE(particles_check_arrays)(Particles,info)
    !!!------------------------------------------------------------------------!
...
END SUBROUTINE DTYPE(particles_check_arrays)

SUBROUTINE DTYPE(particles_add)(Particles1,Particles2,info)
    !!!------------------------------------------------------------------------!
    ...
    !-------------------------------------------------------------------------
    DEFINE_MK()
    TYPE(DTYPE(ppm_t_particles)),POINTER, INTENT(INOUT)    :: Particles1
    !!! Particles
    TYPE(DTYPE(ppm_t_particles)),POINTER, INTENT(IN   )    :: Particles2
    ...
    DO i=1,Particles1%max_wpiid
        IF (Particles1%wpi(i)%is_mapped) THEN
            IF (Particles1%wpi(i)%has_ghosts) THEN
                size_theo=Particles1%Mpart
            ELSE
                size_theo=Particles1%Npart
            ENDIF
            ok = (SIZE(Particles1%wpi(i)%vec).GE.(size_theo))
            IF (.NOT.ok) THEN
                info = ppm_error_error
                CALL ppm_error(999,caller,'wpi too small',&
                    __LINE__,info)
        ...
        ENDIF
    ENDDO
    DO i=1,Particles1%max_wpsid
        IF (Particles1%wps(i)%is_mapped) THEN
            IF (Particles1%wps(i)%has_ghosts) THEN
                size_theo=Particles1%Mpart
            ELSE
                size_theo=Particles1%Npart
            ENDIF
            ok = (SIZE(Particles1%wps(i)%vec).GE.(size_theo))
            IF (.NOT.ok) THEN
                info = ppm_error_error
                CALL ppm_error(999,caller,'wps too small',&
                    __LINE__,info)
                GOTO 9999
            ENDIF
            ok = (.NOT. ANY(ISNAN(Particles1%wps(i)%vec)))
            IF (.NOT.ok) THEN
                info = ppm_error_error
                CALL ppm_error(999,caller,'NaN in wps',&
                    __LINE__,info)
        ...
        ENDIF
    ENDDO
    DO i=1,Particles1%max_wpvid
        IF (Particles1%wpv(i)%is_mapped) THEN
            IF (Particles1%wpv(i)%has_ghosts) THEN
                size_theo=Particles1%wpv(i)%lda * Particles1%Mpart
            ELSE
                size_theo=Particles1%wpv(i)%lda * Particles1%Npart
            ENDIF
            ok = (SIZE(Particles1%wpv(i)%vec).GE.(size_theo))
            IF (.NOT.ok) THEN
                info = ppm_error_error
                CALL ppm_error(999,caller,'wpv too small',&
                    __LINE__,info)
                GOTO 9999
            ENDIF
            ok = (.NOT. ANY(ISNAN(Particles1%wpv(i)%vec)))
            IF (.NOT.ok) THEN
                info = ppm_error_error
                CALL ppm_error(999,caller,'NaN in wpv',&
                    __LINE__,info)
    ...
    IF (Particles1%neighlists) THEN
        size_theo=Particles1%Npart
        ok = (SIZE(Particles1%nvlist).GE.(size_theo))
        IF (.NOT.ok) THEN
            info = ppm_error_error
            CALL ppm_error(999,caller,'nvlist too small',&
                __LINE__,info)
            GOTO 9999
        ENDIF
        ok = (SIZE(Particles1%vlist,2).GE.(size_theo))
        IF (.NOT.ok) THEN
            info = ppm_error_error
            CALL ppm_error(999,caller,'vlist too small',&
                __LINE__,info)
...
END SUBROUTINE DTYPE(particles_add)
