...
minclude ppm_create_collection_interfaces(DTYPE(part_prop)_,DTYPE(part_prop)_)
      ...
      SUBROUTINE DTYPE(prop_create)_(prop,datatype,parts,npart,lda,name,flags,info,field,zero)
          ...
          IMPORT ppm_t_discr_kind,ppm_t_field_
          IMPLICIT NONE
      ...
      &          discr_data,dtype,name,lda,zero,with_ghosts)
          ...
          !CLASS(DTYPE(ppm_t_part_prop)_),OPTIONAL, POINTER       :: discr_data
...
minclude ppm_create_collection_interfaces(DTYPE(neighlist)_,DTYPE(neighlist)_)
      ...
      SUBROUTINE DTYPE(part_remesh)_(this,Mesh,kernel,info, &
      &          p2m_bcdef,cutoff_val,cutoff_field)
          IMPORT DTYPE(ppm_t_particles)_,ppm_t_field_
          IMPORT ppm_t_equi_mesh_, MK
          IMPLICIT NONE
          CLASS(DTYPE(ppm_t_particles)_)                  :: this
          CLASS(ppm_t_equi_mesh_),          INTENT(INOUT) :: Mesh
          INTEGER,                          INTENT(IN   ) :: kernel
      ...
      SUBROUTINE DTYPE(particles_from_mesh)_(this,Mesh,info,cutoff_val,cutoff_field)
      ...
      SUBROUTINE DTYPE(part_interp_to_mesh_all)_(this,Mesh,kernel,info,p2m_bcdef)
          IMPORT DTYPE(ppm_t_particles)_,ppm_t_field_,ppm_t_equi_mesh_
          IMPLICIT NONE
          CLASS(DTYPE(ppm_t_particles)_)                 :: this
          CLASS(ppm_t_equi_mesh_),         INTENT(INOUT) :: Mesh
