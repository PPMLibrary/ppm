      ...
      ENDDO
      ppm_nrecvbuffer = ppm_nrecvbuffer - INT(i,ppm_kind_int64)*INT(bdim,ppm_kind_int64)
      ...
      !-------------------------------------------------------------------------
      ppm_nsendbuffer = ppm_nsendbuffer - INT(ppm_buffer_dim(ppm_buffer_set),ppm_kind_int64)* &
...
!we should put a vector directive here !
                  ...
                  & CMPLX(ppm_recvbufferd(ibuffer-1_ppm_kind_int64),ppm_recvbufferd(ibuffer),ppm_kind_single)
#elif  __KIND == __DOUBLE_PRECISION_COMPLEX
                  pdata(1,ipart) = pdata(1,ipart) + &
                  & CMPLX(ppm_recvbufferd(ibuffer-1_ppm_kind_int64),ppm_recvbufferd(ibuffer),ppm_kind_double)
#elif  __KIND == __INTEGER
...
#elif  __KIND == __SINGLE_PRECISION_COMPLEX
                  pdata(1,ipart) = pdata(1,ipart) + CMPLX(ppm_recvbufferd(ibuffer-1_ppm_kind_int64),    &
     &                ppm_recvbufferd(ibuffer),ppm_kind_single)
                  ibuffer = ibuffer + 2_ppm_kind_int64
                  pdata(2,ipart) = pdata(2,ipart) + CMPLX(ppm_recvbufferd(ibuffer-1_ppm_kind_int64),    &
     &                ppm_recvbufferd(ibuffer),ppm_kind_single)
                  ibuffer = ibuffer + 2_ppm_kind_int64
                  pdata(3,ipart) = pdata(3,ipart) + CMPLX(ppm_recvbufferd(ibuffer-1_ppm_kind_int64),    &
     &                ppm_recvbufferd(ibuffer),ppm_kind_single)
#elif  __KIND == __DOUBLE_PRECISION_COMPLEX
                  pdata(1,ipart) = pdata(1,ipart) + CMPLX(ppm_recvbufferd(ibuffer-1_ppm_kind_int64),    &
...
! if we are going to run this efficiently on ivos machine, we should put a vector directive here !
     ...
     &                ppm_recvbufferd(ibuffer),ppm_kind_single)
                  ibuffer = ibuffer + 2_ppm_kind_int64
                  pdata(4,ipart) = pdata(4,ipart) + CMPLX(ppm_recvbufferd(ibuffer-1_ppm_kind_int64),    &
     &                ppm_recvbufferd(ibuffer),ppm_kind_single)
#elif  __KIND == __DOUBLE_PRECISION_COMPLEX
                  pdata(1,ipart) = pdata(1,ipart) + CMPLX(ppm_recvbufferd(ibuffer-1_ppm_kind_int64),    &
     &                ppm_recvbufferd(ibuffer),ppm_kind_double)
...
! if we are going to run this efficiently on ivos machine, we should put a vector directive here !
     ...
     &                ppm_recvbufferd(ibuffer),ppm_kind_single)
                  ibuffer = ibuffer + 2_ppm_kind_int64
                  pdata(5,ipart) = pdata(5,ipart) + CMPLX(ppm_recvbufferd(ibuffer-1_ppm_kind_int64),    &
     &                ppm_recvbufferd(ibuffer),ppm_kind_single)
#elif  __KIND == __DOUBLE_PRECISION_COMPLEX
                  pdata(1,ipart) = pdata(1,ipart) + CMPLX(ppm_recvbufferd(ibuffer-1_ppm_kind_int64),    &
     &                ppm_recvbufferd(ibuffer),ppm_kind_double)
...
#elif  __KIND == __SINGLE_PRECISION_COMPLEX
                     pdata(i,ipart) = pdata(i,ipart) + CMPLX(ppm_recvbufferd(ibuffer-1_ppm_kind_int64),    &
     &                   ppm_recvbufferd(ibuffer),ppm_kind_single)
#elif  __KIND == __DOUBLE_PRECISION_COMPLEX
                     pdata(i,ipart) = pdata(i,ipart) + CMPLX(ppm_recvbufferd(ibuffer-1_ppm_kind_int64),    &
     &                   ppm_recvbufferd(ibuffer),ppm_kind_double)
...
#elif  __KIND == __DOUBLE_PRECISION_COMPLEX
                  pdata(1,ipart) = pdata(1,ipart) + CMPLX(ppm_recvbuffers(ibuffer-1_ppm_kind_int64),    &
...
! if we are going to run this efficiently on ivos machine, we should put a vector directive here !
     ...
     &                ppm_recvbuffers(ibuffer),ppm_kind_double)
                  ibuffer = ibuffer + 2_ppm_kind_int64
                  pdata(2,ipart) = pdata(2,ipart) + CMPLX(ppm_recvbuffers(ibuffer-1_ppm_kind_int64),    &
     &                ppm_recvbuffers(ibuffer),ppm_kind_double)
#elif  __KIND == __SINGLE_PRECISION_COMPLEX
                  pdata(1,ipart) = pdata(1,ipart) + CMPLX(ppm_recvbuffers(ibuffer-1_ppm_kind_int64),    &
     &                ppm_recvbuffers(ibuffer),ppm_kind_single)
...
! if we are going to run this efficiently on ivos machine, we should put a vector directive here !
     ...
     &                ppm_recvbuffers(ibuffer),ppm_kind_double)
                  ibuffer = ibuffer + 2_ppm_kind_int64
                  pdata(3,ipart) = pdata(3,ipart) + CMPLX(ppm_recvbuffers(ibuffer-1_ppm_kind_int64),    &
     &                ppm_recvbuffers(ibuffer),ppm_kind_double)
#elif  __KIND == __SINGLE_PRECISION_COMPLEX
                  pdata(1,ipart) = pdata(1,ipart) + CMPLX(ppm_recvbuffers(ibuffer-1_ppm_kind_int64),    &
     &                ppm_recvbuffers(ibuffer),ppm_kind_single)
...
! if we are going to run this efficiently on ivos machine, we should put a vector directive here !
     ...
     &                ppm_recvbuffers(ibuffer),ppm_kind_double)
                  ibuffer = ibuffer + 2_ppm_kind_int64
                  pdata(4,ipart) = pdata(4,ipart) + CMPLX(ppm_recvbuffers(ibuffer-1_ppm_kind_int64),    &
     &                ppm_recvbuffers(ibuffer),ppm_kind_double)
#elif  __KIND == __SINGLE_PRECISION_COMPLEX
                  pdata(1,ipart) = pdata(1,ipart) + CMPLX(ppm_recvbuffers(ibuffer-1_ppm_kind_int64),    &
     &                ppm_recvbuffers(ibuffer),ppm_kind_single)
...
! if we are going to run this efficiently on ivos machine, we should put a vector directive here !
     ...
     &                ppm_recvbuffers(ibuffer),ppm_kind_double)
                  ibuffer = ibuffer + 2_ppm_kind_int64
                  pdata(5,ipart) = pdata(5,ipart) + CMPLX(ppm_recvbuffers(ibuffer-1_ppm_kind_int64),    &
     &                ppm_recvbuffers(ibuffer),ppm_kind_double)
#elif  __KIND == __SINGLE_PRECISION_COMPLEX
                  pdata(1,ipart) = pdata(1,ipart) + CMPLX(ppm_recvbuffers(ibuffer-1_ppm_kind_int64),    &
     &                ppm_recvbuffers(ibuffer),ppm_kind_single)
            ...
            DO k=1,ppm_nsendlist ! or ppm_nrecvlist or ppm_ncommseq(topoid)
               DO j=1,ppm_ghosthack(1,k)
                  ...
                  DO i=1,edim
                     ...
                     & CMPLX(ppm_recvbuffers(ibuffer-1_ppm_kind_int64),ppm_recvbuffers(ibuffer),ppm_kind_double)
#elif  __KIND == __SINGLE_PRECISION_COMPLEX
                     pdata(i,ipart) = pdata(i,ipart) + &
                     & CMPLX(ppm_recvbuffers(ibuffer-1_ppm_kind_int64),ppm_recvbuffers(ibuffer),ppm_kind_single)
#elif  __KIND == __INTEGER
