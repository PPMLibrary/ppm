      ...
      SUBROUTINE ppm_map_part_isend(Npart,Mpart,info,sendrecv)
         ...
         DO k=2,ppm_nsendlist
            ...
            IF (ppm_isendlist(k).GE.0.AND.ppm_irecvlist(k).GE.0) THEN
              IF (ppm_rank.GT.ppm_irecvlist(k)) THEN
               ...
               &    tag,ppm_comm,ppm_request(ppm_nrequest),info)
            ...
            DO k=1,ppm_buffer_set
               ...
               DO i=2,ppm_nsendlist
                  ...
                  IF (precv(i).GT.0) THEN
                     ...
                     &    ppm_mpi_kind,ppm_irecvlist(i),tag,ppm_comm,ppm_request(ppm_nrequest),info)
                     or_fail_MPI("MPI_Irecv")
                  ENDIF
                  IF (psend(i).GT.0) THEN
                     ...
                     &    ppm_mpi_kind,ppm_isendlist(i),tag,ppm_comm,ppm_request(ppm_nrequest),info)
                     or_fail_MPI("MPI_Isend")
                  ENDIF
