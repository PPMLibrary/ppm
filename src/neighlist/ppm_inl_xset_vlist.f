     ...
     !-------------------------------------------------------------------------
...
#if   __MODE == __INL
      SUBROUTINE inl_xset_vlist_s(topoid,red,nred,mred,blue,nblue,mblue, &
      &          rcblue,skin,ghostlayer,info,vlist,nvlist,lstore)
#elif __MODE == __HNL
      SUBROUTINE hnl_xset_vlist_s(topoid,red,nred,mred,blue,nblue,mblue, &
      &          cutoff,skin,ghostlayer,info,vlist,nvlist,lstore)
#endif
#elif __KIND == __DOUBLE_PRECISION
#if   __MODE == __INL
      SUBROUTINE inl_xset_vlist_d(topoid,red,nred,mred,blue,nblue,mblue, &
      &          rcblue,skin,ghostlayer,info,vlist,nvlist,lstore)
#elif __MODE == __HNL
      SUBROUTINE hnl_xset_vlist_d(topoid,red,nred,mred,blue,nblue,mblue, &
      &          cutoff,skin,ghostlayer,info,vlist,nvlist,lstore)
#endif
#endif
      ...
      REAL(MK), DIMENSION(:,:), POINTER     :: red_sub
...
#if   __MODE == __HNL
      REAL(MK), DIMENSION(:), POINTER, SAVE :: rcblue => NULL()
#endif
      INTEGER , DIMENSION(:,:), POINTER     :: vlist_sub
      INTEGER , DIMENSION(:)  , POINTER     :: nvlist_sub
      INTEGER , DIMENSION(:)  , POINTER     :: red_p_id
      INTEGER , DIMENSION(:)  , POINTER     :: blue_p_id
      INTEGER                               :: nred_sub
      INTEGER                               :: mred_sub
      INTEGER                               :: nblue_sub
      INTEGER                               :: mblue_sub
      ...
      CHARACTER(LEN=ppm_char) :: caller='xset_vlist'
      ...
      or_fail_alloc('rcblue',exit_point=no,ppm_error=ppm_error_fatal)
      ...
      or_fail_alloc('rcred',exit_point=no,ppm_error=ppm_error_fatal)
      ...
      DO rank_sub = 1, topo%nsublist
         ...
         ! of this subdomain.
         !-----------------------------------------------------------------
         NULLIFY(red_sub,blue_sub,rcred_sub,rcblue_sub,red_p_id,blue_p_id)

         CALL getSubdomainParticles(red,nred,mred,rcred,.FALSE., &
         &    curr_sub,ghostlayer,red_sub,rcred_sub,nred_sub,    &
         &    mred_sub,red_p_id)

         CALL getSubdomainParticles(blue,nblue,mblue,rcblue,.FALSE., &
         &    curr_sub,ghostlayer,blue_sub,rcblue_sub,nblue_sub,     &
         &    mblue_sub,blue_p_id)
         ...
         &    blue_sub,nblue_sub,mblue_sub,rcblue_sub,skin,curr_sub,     &
         &    ghostlayer,info,vlist_sub,nvlist_sub,lst)
      ...
      &          mblue,rcblue,skin,curr_dom,ghostlayer,info,vlist,nvlist,lstore)
#elif __KIND == __DOUBLE_PRECISION
      ...
      &          mblue,rcblue,skin,curr_dom,ghostlayer,info,vlist,nvlist,lstore)
        ...
        CHARACTER(LEN=ppm_char) :: caller='create_xset_vlist'
        ...
        or_fail_alloc('own_red',ppm_error=ppm_error_fatal)

        CALL ppm_alloc(own_blue, lda, iopt, info)
        or_fail_alloc('own_blue',ppm_error=ppm_error_fatal)
        ...
        or_fail_alloc('neigh_red',ppm_error=ppm_error_fatal)

        CALL ppm_alloc(neigh_blue, lda, iopt, info)
        or_fail_alloc('neigh_blue',ppm_error=ppm_error_fatal)
        ...
        &   skin, whole_domain,curr_dom,vlist,nvlist,lst,info)
        or_fail('get_xset_VerletLists')
        ...
        !-------------------------------------------------------------------------
        CALL ppm_alloc(own_red, lda, iopt, info)
        or_fail_dealloc('own_red',ppm_error=ppm_error_fatal)

        CALL ppm_alloc(own_blue, lda, iopt, info)
        or_fail_dealloc('own_blue',ppm_error=ppm_error_fatal)

        !-------------------------------------------------------------------------
        !  Deallocate neigh_plist.
        !-------------------------------------------------------------------------
        CALL ppm_alloc(neigh_red, lda, iopt, info)
        or_fail_dealloc('neigh_red',ppm_error=ppm_error_fatal)

        CALL ppm_alloc(neigh_blue, lda, iopt, info)
        or_fail_dealloc('neigh_blue',ppm_error=ppm_error_fatal)

        CALL ppm_destroy_inl_clist(red_clist,info)
        or_fail('ppm_destroy_inl_clist red',ppm_error=ppm_error_fatal)

        CALL ppm_destroy_inl_clist(blue_clist,info)
        or_fail('ppm_destroy_inl_clist blue',ppm_error=ppm_error_fatal)
      ...
      &          blue_clist, skin, whole_domain,curr_dom,vlist,nvlist,lstore,info)
#elif __KIND == __DOUBLE_PRECISION
      ...
      &          blue_clist,skin,whole_domain,curr_dom,vlist,nvlist,lstore,info)
        ...
        CHARACTER(LEN=ppm_char) :: caller='get_xset_VerletLists'
        ...
        DO i = 1, red_clist%n_real_p
           ...
           &    whole_domain,red,rcred,blue,rcblue,skin,nvlist)
        ENDDO
        ...
        IF (lstore) THEN
           ...
           DO i = 1, red_clist%n_real_p
              ...
              &    whole_domain,red,rcred,blue,rcblue,skin,vlist,nvlist)
           ENDDO
        ENDIF
