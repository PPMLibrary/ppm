      ...
      SUBROUTINE cnl_rank2d_s(xp,np,xmin,xmax,nm,ngl,lpdx,lhbx,info)
#elif __KIND == __DOUBLE_PRECISION
      SUBROUTINE cnl_rank2d_d(xp,np,xmin,xmax,nm,ngl,lpdx,lhbx,info)
#endif
      ...
      INTEGER(ppm_kind_int64) , DIMENSION(:)  , INTENT(IN   ) :: nm
      ...
      INTEGER(ppm_kind_int64) , DIMENSION(:)  , POINTER       :: lpdx
      !!! Index of particles in cells (lpdx(icnt))
      INTEGER(ppm_kind_int64) , DIMENSION(:)  , POINTER       :: lhbx
      ...
      INTEGER                                 :: info2
      ! counters
      INTEGER(ppm_kind_int64)                                 :: i,j,icount,ipart,icorr
      INTEGER(ppm_kind_int64)                                 :: nbox,ibox
      ! work arrays: box idx of each particle, write pointer, number of
      ! particles per box
      INTEGER(ppm_kind_int64), DIMENSION(:), POINTER          :: pbox  => NULL()
      INTEGER(ppm_kind_int64), DIMENSION(:), POINTER          :: cbox  => NULL()
      INTEGER(ppm_kind_int64), DIMENSION(:), POINTER          :: npbx  => NULL()
      ! total number of cells in each direction (including ghost layers)
      INTEGER(ppm_kind_int64), DIMENSION(2)                   :: nmtot
      ...
      info2 = info
      CALL substart('ppm_cnl_rank2d',t0,info)
      ...
      IF (info .NE. 0) THEN
         ...
         CALL ppm_error(ppm_err_alloc,'ppm_cnl_rank2d',     &
     ...
     &          xp(2,ipart).LT.xmin(2).OR.xp(2,ipart).GE.xmax(2)) THEN
         ...
         IF (icount .GT. 0) THEN
            ...
            CALL ppm_error(ppm_err_part_range,'ppm_cnl_rank2d',msg,  &
     &          __LINE__,info)
         ENDIF
      ENDIF
     ...
     &       (j .GE. 0 .AND. j .LT. nmtot(2))) THEN
      ...
      IF (icorr.GT.0) THEN
         ...
         CALL ppm_error(ppm_err_index_corr,'ppm_cnl_rank2d',msg,  &
     &                  __LINE__,info)
      ENDIF
     ...
     &        'particle index list LPDX',__LINE__,info)
      ...
      IF (info .NE. 0) THEN
         info = ppm_error_error
         CALL ppm_error(ppm_err_dealloc,'ppm_cnl_rank2d',     &
     &        'number of particles per box NPBX',__LINE__,info)
      ENDIF
     ...
     &        'box index of particle PBOX',__LINE__,info)
      ...
      !-------------------------------------------------------------------------
 9999 CONTINUE
      CALL substop('ppm_cnl_rank2d',t0,info)
      RETURN
      CONTAINS
      SUBROUTINE check
         IF (np .LT. 0) THEN
            info = ppm_error_error
            CALL ppm_error(ppm_err_argument,'ppm_cnl_rank2d',  &
...
#if   __KIND == __SINGLE_PRECISION
      END SUBROUTINE cnl_rank2d_s
#elif __KIND == __DOUBLE_PRECISION
      END SUBROUTINE cnl_rank2d_d
#endif
