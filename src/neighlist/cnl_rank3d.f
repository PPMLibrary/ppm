      ...
      SUBROUTINE cnl_rank3d_s(xp,Np,xmin,xmax,nm,ngl,lpdx,lhbx,info)
#elif __KIND == __DOUBLE_PRECISION
      SUBROUTINE cnl_rank3d_d(xp,Np,xmin,xmax,nm,ngl,lpdx,lhbx,info)
#endif
      ...
      INTEGER                                 :: info2
      ! counters
      INTEGER(ppm_kind_int64)                                 :: i,j,k,icount,ipart,icorr
      INTEGER(ppm_kind_int64)                                 :: n2,nbox,ibox
      ...
      INTEGER(ppm_kind_int64), DIMENSION(3)                   :: nmtot
      ...
      info2 = info
      CALL substart('ppm_cnl_rank3d',t0,info)
      ...
      IF (info .NE. 0) THEN
         ...
         CALL ppm_error(ppm_err_alloc,'ppm_cnl_rank3d',     &
     ...
     &          xp(3,ipart).LT.xmin(3).OR.xp(3,ipart).GE.xmax(3)) THEN
         ...
         IF (icount .GT. 0) THEN
            ...
            CALL ppm_error(ppm_err_part_range,'ppm_cnl_rank3d',msg,   &
     &          __LINE__,info)
         ENDIF
      ENDIF
     ...
     &       (k .GE. 0 .AND. k .LT. nmtot(3))) THEN
      ...
      IF (icorr.GT.0) THEN
         ...
         CALL ppm_error(ppm_err_index_corr,'ppm_cnl_rank3d',msg,  &
     &        __LINE__,info)
      ENDIF
     ...
     &        'particle index list LPDX',__LINE__,info)
      ...
      IF (info .NE. 0) THEN
         info = ppm_error_error
         CALL ppm_error(ppm_err_dealloc,'ppm_cnl_rank3d',     &
     &        'number of particles per box NPBX',__LINE__,info)
      ENDIF
      CALL ppm_alloc(cbox,ldc,iopt,info)
      IF (info .NE. 0) THEN
     ...
     &        'box index of particle PBOX',__LINE__,info)
      ...
      !-------------------------------------------------------------------------
 9999 CONTINUE
      CALL substop('ppm_cnl_rank3d',t0,info)
      RETURN
      CONTAINS
      SUBROUTINE check
         IF (Np .LT. 0) THEN
            info = ppm_error_error
            CALL ppm_error(ppm_err_argument,'ppm_cnl_rank3d',  &
...
#if   __KIND == __SINGLE_PRECISION
      END SUBROUTINE cnl_rank3d_s
#elif __KIND == __DOUBLE_PRECISION
      END SUBROUTINE cnl_rank3d_d
#endif
