     ...
     !-------------------------------------------------------------------------
      ...
      CHARACTER(LEN=ppm_char) :: caller='create_inl_clist'
      ...
      or_fail_alloc('clist%rank',ppm_error=ppm_error_fatal)
      ...
      IF (ppm_dim.EQ.2) THEN
         ...
         or_fail_alloc("borders",ppm_error=ppm_error_fatal)
      ...
      FORALL (i=1:clist%n_all_p) clist%rank(i)=i
      ...
      !-------------------------------------------------------------------------
      CALL sortByRC(cutoff,clist%rank)
      ...
      or_fail_alloc("rc_borders",ppm_error=ppm_error_fatal)
      ...
      &   (clist%rc_borders(1)-1)),clist,whole_domain,idx0,1,clist%rc_borders(0)-1)

      DO level = 2, clist%max_depth
         CALL SortByRC_Pos(xp,cutoff,skin,clist%rank(clist%rc_borders(level-1): &
         &    (clist%rc_borders(level)-1)),clist,whole_domain,idx,level,        &
         &    clist%rc_borders(level-1)-1)
      ENDDO
      ...
      or_fail_dealloc('rc_borders',ppm_error=ppm_error_fatal)
      ...
      CHARACTER(LEN=ppm_char) :: caller='ppm_destroy_inl_clist'
      ...
      !-------------------------------------------------------------------------
      CALL ppm_alloc(clist%borders, lda, iopt, info)
      or_fail_dealloc("borders",ppm_error=ppm_error_fatal)

      !-------------------------------------------------------------------------
      !  Deallocate rank array.
      !-------------------------------------------------------------------------
      CALL ppm_alloc(clist%rank,lda, iopt, info)
      or_fail_dealloc("rank",ppm_error=ppm_error_fatal)

      !-------------------------------------------------------------------------
      !  Deallocate rankByPos array.
      !-------------------------------------------------------------------------
      CALL ppm_alloc(clist%rankByPos,  lda, iopt, info)
      or_fail_dealloc("rankByPos",ppm_error=ppm_error_fatal)
      ...
      or_fail_dealloc("Could not destroy htable",ppm_error=ppm_error_fatal)
      ...
      !!! Given the index of parent cell, returns the index of its first child.
      !!! Works for nD.
      IMPLICIT NONE
      !---------------------------------------------------------------------
      !  Arguments
...
#if __KIND == __SINGLE_PRECISION
#ifdef __DEBUG
      FUNCTION isEmpty(c_idx,lookup_) RESULT(empty)
#else
      PURE FUNCTION isEmpty(c_idx,lookup_) RESULT(empty)
#endif
      ...
      !!! Input index
      TYPE(ppm_htable),        INTENT(IN   ) :: lookup_
      !!! hash table
      logical                                :: empty
      !!! Logical result
      !---------------------------------------------------------------------
      ...
      borders_pos = lookup_%search(parentIdx) ! Search parent in hash table
      empty =MERGE(.TRUE.,.FALSE.,borders_pos.EQ.htable_null)
      ...
      CHARACTER(LEN=ppm_char) :: caller='setSubregions'
      ...
      DO i = 1, ppm_dim
         ...
         mid_coor(i)   = (start_coor(i) + end_coor(i))/2.0_MK
      ENDDO
      ...
      CHARACTER(LEN=ppm_char) :: caller='SortByPosition'
      ...
      ! Get the minimum cutoff radius + skin
      ...
      IF (clist%lookup%nrow.GT.s) THEN
         ...
         IF (ppm_dim.EQ.2) THEN
            ...
            ! Allocate "borders" array for 2D case
            CALL ppm_alloc(clist%borders,lda,iopt,info)
            or_fail_alloc("borders",exit_point=no,ppm_error=ppm_error_fatal)
         ELSEIF (ppm_dim.EQ.3) THEN
            lda(1) = 10
            ! Allocate "borders" array for 3D case
            CALL ppm_alloc(clist%borders,lda,iopt,info)
         ...
         FORALL (i=1:lda(1),j=s+1:lda(2)) clist%borders(i,j)=0
      ...
      !Yaser
      !it seems a subtle bug in the code, so I added this part here
      ! Get minimum side length
      minSideLength = getMinimumSideLength(ownregion)
      ...
      IF (getCellDepth(idx) + 1.GE.level)  THEN
         ...
         IF (ppm_dim.EQ.2)       THEN
            ...
            clist%borders(5, clist%borders_pos) = s       + increment
         ELSEIF (ppm_dim.EQ.3)   THEN
            clist%borders(1, clist%borders_pos) =                 increment
            ...
            clist%borders(9, clist%borders_pos) = s             + increment
         ENDIF
...
#if   __KIND == __SINGLE_PRECISION
      RECURSIVE SUBROUTINE sortByRC_s(cutoff,rank)
#elif __KIND == __DOUBLE_PRECISION
      RECURSIVE SUBROUTINE sortByRC_d(cutoff,rank)
#endif
      !!! Recursive algorithm which sorts particles by their cutoff
      !!! radii in descending order; given cutoff radii, and the
      ...
      IF (SIZE(rank).GT.1) THEN
         CALL partitionByRC(cutoff,rank,marker)
         CALL sortByRC(cutoff,rank(:marker-1))
         CALL sortByRC(cutoff,rank(marker:))
      ENDIF
#if   __KIND == __SINGLE_PRECISION
...
#if   __KIND == __SINGLE_PRECISION
      SUBROUTINE partitionByRC_s(cutoff,rank,marker)
#elif __KIND == __DOUBLE_PRECISION
      SUBROUTINE partitionByRC_d(cutoff,rank,marker)
#endif
      ...
      CHARACTER(LEN=ppm_char) :: caller='getRC_Borders'
      ...
      DO i = 1, clist%max_depth
         ...
         ! minimum side length after a subdivision
