     ...
     !-------------------------------------------------------------------------
      ...
      SUBROUTINE inl_k_vlist_s(topoid, xp, Np, Mp, cutoff, knn, lsymm,    &
     & ghostlayer, info, vlist, nvlist, lstore)
#elif __KIND == __DOUBLE_PRECISION
      SUBROUTINE inl_k_vlist_d(topoid, xp, Np, Mp, cutoff, knn, lsymm,    &
     & ghostlayer, info, vlist, nvlist, lstore)
#endif
      IMPLICIT NONE
#if   __KIND == __SINGLE_PRECISION
      ...
      CALL substart('ppm_inl_k_vlist',t0,info)
      ...
      IF (info .NE. 0) THEN
          ...
          CALL ppm_error(ppm_err_alloc,'ppm_inl_k_vlist',     &
 ...
 &                 ghostlayer, xp_sub, cutoff_sub, Np_sub,Mp_sub,p_id)
     ...
     &             knn, lsymm, actual_subdomain, ghostlayer, info, vlist_sub,&
     &             nvlist_sub,lst)
          ...
          END IF
      ENDDO
      CALL substop('ppm_inl_k_vlist',t0,info)
#if   __KIND == __SINGLE_PRECISION
      END SUBROUTINE inl_k_vlist_s
#elif __KIND == __DOUBLE_PRECISION
      END SUBROUTINE inl_k_vlist_d
#endif

#if   __KIND == __SINGLE_PRECISION
      SUBROUTINE create_inl_vlist_s(xp, Np, Mp, cutoff, knn, lsymm, &
     & actual_domain, ghostlayer, info, vlist, nvlist, lstore)
#elif __KIND == __DOUBLE_PRECISION
      SUBROUTINE create_inl_vlist_d(xp, Np, Mp, cutoff, knn, lsymm, &
     & actual_domain, ghostlayer, info, vlist, nvlist, lstore)
#endif
      ...
      !<<<<<<<<<<<<<<<<<<<<<<<<< Start of the code >>>>>>>>>>>>>>>>>>>>>>>>>!
          CALL substart('create_inl_vlist',t0,info)
     ...
     &                       'empty_list',__LINE__,info)
      ...
      !-------------------------------------------------------------------------
          ...
          CALL getVerletLists(xp, cutoff, clist, knn, lsymm, whole_domain, &
     ...
     &                       'ppm_destroy_inl_clist',__LINE__,info)
      ...
      !-------------------------------------------------------------------------
      !  Deallocate dist_array and dist_rank
      !-------------------------------------------------------------------------
#if   __KIND == __SINGLE_PRECISION
      CALL ppm_alloc(dist_array_s, lda, iopt, info)
#elif __KIND == __DOUBLE_PRECISION
      CALL ppm_alloc(dist_array_d, lda, iopt, info)
#endif
      IF (info.NE.0) THEN
          info = ppm_error_fatal
          CALL ppm_error(ppm_err_dealloc,'create_inl_vlist',     &
              &                       'dist_array',__LINE__,info)
          GOTO 9999
      END IF
      
      CALL ppm_alloc(dist_rank, lda, iopt, info)
      IF (info.NE.0) THEN
          info = ppm_error_fatal
          CALL ppm_error(ppm_err_dealloc,'create_inl_vlist',     &
              &                       'dist_rank',__LINE__,info)
          GOTO 9999
      END IF
...
#if   __KIND == __SINGLE_PRECISION
      SUBROUTINE getVerletLists_s(xp, cutoff, clist, knn, lsymm, whole_domain,    &
     & actual_domain, vlist, nvlist, lstore,info)
#elif __KIND == __DOUBLE_PRECISION
      SUBROUTINE getVerletLists_d(xp, cutoff, clist, knn, lsymm, whole_domain,    &
     & actual_domain, vlist, nvlist, lstore,info)
#endif
      ...
      !-------------------------------------------------------------------------
          IF(lstore) THEN
            ...
            &                       'vlist',__LINE__,info)
              ...
              !-----------------------------------------------------------------
              !  Allocate dist_array and dist_rank
              !-----------------------------------------------------------------
              lda(1) = max_nneigh
#if   __KIND == __SINGLE_PRECISION
              CALL ppm_alloc(dist_array_s, lda, iopt, info)
        ...
        &  'ppm_inl_k_vlist not yet implemented for lsymm=.true.',__LINE__,info)
              !DO p_idx = 1, clist%n_all_p
                  ...
                  !& whole_domain, actual_domain, xp, cutoff, knn, vlist, nvlist)
              !END DO
              ELSE                           ! If not symmetric
                  DO p_idx = 1, clist%n_all_p
                      CALL get_neigh(clist%rank(p_idx), clist,whole_domain,  &
                      & xp, cutoff, knn, vlist, nvlist)
                  END DO
              END IF
          END IF
