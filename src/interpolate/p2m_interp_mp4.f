      ...
      REAL(MK), PARAMETER :: four=4.0_MK
      REAL(MK), PARAMETER :: half=0.5_MK
      REAL(MK), PARAMETER :: one5=1.5_MK
      REAL(MK), PARAMETER :: two5=2.5_MK
      ...
      dxxi = one/Mesh%h(1)
      dxyi = one/Mesh%h(2)
      IF (ppm_dim.EQ.3) dxzi = one/Mesh%h(3)
...
!
            ...
            DO jj = -1,2
               ...
               IF (x2.LT.one) THEN
                  wx2 = one - x2**2*(two5-one5*x2)
               ELSE
                  wx2 = two + (-four + (two5 - half * x2)*x2)*x2
               ENDIF
               DO ii = - 1,2
                  ...
                  IF (x1.LT.one) THEN
                     wx1 =  one - x1**2*(two5 - one5*x1)
                  ELSE
                     wx1 =  two + (-four + (two5 - half*x1)*x1)*x1
                  ENDIF

#if __MODE == __SCA
                  field_up(ii+ip1,jj+ip2)= field_up(ii+ip1,jj+ip2) + wx1*wx2*up(iq)
#else
                  DO ldn=1,lda
                     field_up(ldn,ii+ip1,jj+ip2)= field_up(ldn,ii+ip1,jj+ip2) + wx1*wx2*up(ldn,iq)
                  ENDDO
#endif
               ENDDO !ii
            ENDDO !jj
