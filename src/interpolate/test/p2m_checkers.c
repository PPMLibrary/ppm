...
void createRandomParticlePosition_2d(
    ...
    for(k = 0; k < p_per_cell; k++){
        for(y = 0; y < mesh_size[1] - 0; y++){
            for(x = 0; x < mesh_size[0] - 0; x++){
...
void createRandomParticleMass_2d(
	...
	for(k = 0; k < p_per_cell; k++){
		for(y = 0; y < mesh_size[1]; y++){
			for(y = 0; y < mesh_size[1]; y++){
				for(x = 0; x < mesh_size[0]; x++){
					for(p_mass = 0; p_mass < nmass; p_mass++){
						mass[nmass*p + p_mass] = ((rand()%10001 - 5000)/10000.0);
					}
					p++;
				}
			}
...
void check_interpolation_2d_p2m_mp4_(
    ...
    grid_seq = (float *)malloc(sizeof(float)*ngrid*nmass);

    for(g_id = 0; g_id < ngrid*nmass; g_id++){
        grid_seq[g_id] = 0.0f;
    ...
    for(p = 0; p < nparticle; p++){
        ...
        for (y = -1; y < 3; y++){
            ...
            for (x = -1; x < 3; x++){
                ...
                cell     = (ip1 + x) + (ip2 + y)*mesh_size[0];
                
                if( (c_x_coor > physmax[0]) ||
                    (c_x_coor < physmin[0]) ||
                    (c_y_coor < physmin[1]) ||
                    (c_y_coor > physmax[1])){ continue;}

				for(particle_mass = 0; particle_mass < nmass; particle_mass++){
                	grid_seq[nmass*cell + particle_mass] += mass[nmass*p + particle_mass]*wx1*wx2;
				}
/*
				if(cell == 74147){
					printf("p: %d \n", p);
...
*/
    ...
    for(g_id = 0; g_id < ngrid*nmass; g_id++){
        if(fabs(grid[g_id] - grid_seq[g_id]) > 0.01){ // HERE IS IN QUESTION!
            printf("Interpolation is wrong at g_id: %d \n", g_id);
            printf("grid[g_id]: %12.8f, grid_seq[g_id]: %12.8f \n", grid[g_id], grid_seq[g_id]);

            for(g_id2 = 0; g_id2 <= g_id + 10; g_id2++){
                printf("grid[%d]: %f grid_seq[%d]: %f \n", g_id2, grid[g_id2], g_id2, grid_seq[g_id2]);
            }
            exit(0);
        }
    }
        ...
        for (z = -1; z < 3; z++){
            ...
            for (y = -1; y < 3; y++){
                ...
                for (x = -1; x < 3; x++){
                    ...
                    cell     = (ip1 + x) + (ip2 + y)*mesh_size[0] + (ip3 + z)*mesh_size[0]*mesh_size[1];
                ...
                if( (c_x_coor > physmax[0]) ||
                    ...
                    (c_y_coor > physmax[1]) ||
                    (c_z_coor > physmax[2]) ||
                    (c_z_coor < physmin[2])){ continue;}

				for(particle_mass = 0; particle_mass < nmass; particle_mass++){
                	grid_seq[nmass*cell + particle_mass] += mass[nmass*p + particle_mass]*wx1*wx2*wx3;
				}
/*
				if(cell == 0){
					printf("p: %d \n", p);
					printf("xp1, xp2, xp3: %f %f %f \n", xp[3*p], xp[3*p + 1], xp[3*p + 2]);
					printf("mass 1, mass 2: %f %f \n", mass[2*p], mass[2*p + 1]);
				}
*/
				}
            }
...
void check_interpolation_2d_p2m_bsp2_(
	...
	float physmin_with_ghost[2];
	float physmax_with_ghost[2];
    ...
    for(dim = 0; dim < ndim; dim++){
        h[dim] = (physmax_wo_ghost[dim] - physmin_wo_ghost[dim])/(mesh_size[dim] - 1);

		physmin_with_ghost[dim] = physmin_wo_ghost[dim] - h[dim];
		physmax_with_ghost[dim] = physmax_wo_ghost[dim] + h[dim];
	}
    ...
    for(p = 0; p < nparticle; p++){
        x01 = (xp[p*ndim] - physmin_with_ghost[0])/h[0];
        x02 = (xp[p*ndim + 1] - physmin_with_ghost[1])/h[1];
        ...
        for (y = 0; y < 2; y++){
            ...
            for (x = 0; x < 2; x++){
                ...
                c_x_coor = (ip1 + x)*h[0] + physmin_wo_ghost[0];  
                c_y_coor = (ip2 + y)*h[1] + physmin_wo_ghost[1]; 

                cell     = (ip1 + x) + (ip2 + y)*mesh_size[0];
                
                if( (c_x_coor > physmax_wo_ghost[0]) ||
                    (c_x_coor < physmin_wo_ghost[0]) ||
                    (c_y_coor < physmin_wo_ghost[1]) ||
                    (c_y_coor > physmax_wo_ghost[1])){ continue;}
...
/*
					if(cell == 98){
						printf("*******\n");
						printf("%d %lf \n", p, mass[nmass*p + particle_mass]);
						printf("%d %lf %lf \n", p, xp[p*ndim], xp[p*ndim + 1]);
						printf("%d %d %d \n", p, ip1, ip2);
					}
*/
				}
    ...
    for(g_id = 0; g_id < ngrid*nmass; g_id++){
        if(fabs(grid[g_id] - grid_seq[g_id]) > 0.001){ // HERE IS IN QUESTION!
            printf("Interpolation is wrong at g_id: %d \n", g_id);
            printf("grid[g_id]: %12.8f, grid_seq[g_id]: %12.8f \n", grid[g_id], grid_seq[g_id]);
...
void check_interpolation_3d_p2m_bsp2_(
	...
	float physmin_with_ghost[3];
	float physmax_with_ghost[3];
    ...
    for(p = 0; p < nparticle; p++){
        x01 = (xp[p*ndim]     - physmin_with_ghost[0])/h[0];
        x02 = (xp[p*ndim + 1] - physmin_with_ghost[1])/h[1];
        x03 = (xp[p*ndim + 2] - physmin_with_ghost[2])/h[2];
        ...
        for (z = 0; z < 2; z++){
	        ...
	        for (y = 0; y < 2; y++){
            	...
            	for (x = 0; x < 2; x++){
                	...
                	c_x_coor = (ip1 + x)*h[0] + physmin_wo_ghost[0];  
                	c_y_coor = (ip2 + y)*h[1] + physmin_wo_ghost[1]; 
                	c_z_coor = (ip3 + z)*h[2] + physmin_wo_ghost[2]; 
                	...
                	if( (c_x_coor > physmax_wo_ghost[0]) ||
                    	(c_x_coor < physmin_wo_ghost[0]) ||
                    	(c_y_coor < physmin_wo_ghost[1]) ||
                    	(c_y_coor > physmax_wo_ghost[1]) ||
                    	(c_z_coor < physmin_wo_ghost[2]) ||
                    	(c_z_coor > physmax_wo_ghost[2])){ continue;}

					for(particle_mass = 0; particle_mass < nmass; particle_mass++){
                		grid_seq[nmass*cell + particle_mass] += mass[nmass*p + particle_mass]*wx1*wx2*wx3;

						if(cell == 0){
			//				printf("%d %lf %lf %lf %lf\n", p, mass[nmass*p + particle_mass], xp[ndim*p], xp[ndim*p + 1], xp[ndim*p + 2]);
						}
