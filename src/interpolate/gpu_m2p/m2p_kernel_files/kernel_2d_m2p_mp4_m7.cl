...
__kernel void kernel_clist_duplicate_mesh(
	...
	if((t_id[0] < mesh_size[0]) && (t_id[1] < mesh_size[1])){
		...
		mesh_ID_padded = (wg_id[0] + wg_id[1]*num_groups[0])*665 + local_id[0] + local_id[1]*35;
...
__kernel void kernel_interpolate(
	...
	for(i = l_ID; i < 665; i+= 512){
		local_mesh[       i] = mesh_padded[wg_ID*665 + i];
		local_mesh[665  + i] = mesh_padded[nmesh_ext_padded + wg_ID*665 + i];
		local_mesh[1330 + i] = mesh_padded[2*nmesh_ext_padded + wg_ID*665 + i];
		local_mesh[1995 + i] = mesh_padded[3*nmesh_ext_padded + wg_ID*665 + i];
		local_mesh[2660 + i] = mesh_padded[4*nmesh_ext_padded + wg_ID*665 + i];
		local_mesh[3325 + i] = mesh_padded[5*nmesh_ext_padded + wg_ID*665 + i];
		local_mesh[3990 + i] = mesh_padded[6*nmesh_ext_padded + wg_ID*665 + i];
	}
	barrier(CLK_LOCAL_MEM_FENCE);
...
// m_id.x = -1
			...
			particle_mass[1] += wx_x*wx_y*local_mesh[665  + l_ID];
			particle_mass[2] += wx_x*wx_y*local_mesh[1330 + l_ID];
			particle_mass[3] += wx_x*wx_y*local_mesh[1995 + l_ID];
			particle_mass[4] += wx_x*wx_y*local_mesh[2660 + l_ID];
			particle_mass[5] += wx_x*wx_y*local_mesh[3325 + l_ID];
			particle_mass[6] += wx_x*wx_y*local_mesh[3990 + l_ID];
