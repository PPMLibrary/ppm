...
(
	...
	if((t_id[0] < mesh_size[0]) && (t_id[1] < mesh_size[1]) && (t_id[2] < mesh_size[2])){
		...
		mesh_ID_padded = (wg_id[0] + wg_id[1]*num_groups[0] + wg_id[2]*num_groups[0]*num_groups[1])*825 + local_id[0] + local_id[1]*33 + local_id[2]*165;
...
(
	...
	if(p_id < nparticle) {
		...
		cell_ID    = cell_pos_x + 32*cell_pos_y + cell_pos_z*128 +  (wg_x + num_groups[0]*wg_y + num_groups[0]*num_groups[1]*wg_z)*512; 
...
(
	...
	for(i = l_ID; i < 825; i+= 512){
		local_mesh[i]        = mesh_padded[			   		    wg_ID*825 + i];
		local_mesh[i + 825]  = mesh_padded[  nmesh_ext_padded + wg_ID*825 + i];
		local_mesh[i + 1650] = mesh_padded[2*nmesh_ext_padded + wg_ID*825 + i];
		local_mesh[i + 2475] = mesh_padded[3*nmesh_ext_padded + wg_ID*825 + i];
		local_mesh[i + 3300] = mesh_padded[4*nmesh_ext_padded + wg_ID*825 + i];
		local_mesh[i + 4125] = mesh_padded[5*nmesh_ext_padded + wg_ID*825 + i];
	}
	barrier(CLK_LOCAL_MEM_FENCE);
		...
		for(m_id_z = 0; m_id_z < 2; m_id_z++){
			...
			for(m_id_y = 0; m_id_y < 2; m_id_y++){
				...
				for(m_id_x = 0; m_id_x < 2; m_id_x++){	
					...
					particle_mass[1] += wx_x*wx_y*wx_z*local_mesh[l_ID + 825];
					particle_mass[2] += wx_x*wx_y*wx_z*local_mesh[l_ID + 1650];
					particle_mass[3] += wx_x*wx_y*wx_z*local_mesh[l_ID + 2475];
					particle_mass[4] += wx_x*wx_y*wx_z*local_mesh[l_ID + 3300];
					particle_mass[5] += wx_x*wx_y*wx_z*local_mesh[l_ID + 4125];
