...
__kernel void kernel_clist_duplicate_mesh(
	...
	if((t_id[0] < mesh_size[0]) && (t_id[1] < mesh_size[1]) && (t_id[2] < mesh_size[2])){
		...
		mesh_padded[                 mesh_ID_padded] = mesh[4*mesh_ID];
		mesh_padded[  nmesh_padded + mesh_ID_padded] = mesh[4*mesh_ID + 1];
		mesh_padded[2*nmesh_padded + mesh_ID_padded] = mesh[4*mesh_ID + 2];
		mesh_padded[3*nmesh_padded + mesh_ID_padded] = mesh[4*mesh_ID + 3];
	}
}
...
__kernel void kernel_interpolate(
	...
	int   		i, rank, mass;
	int  		m_id_z;
	t_real 		x, x0, x1, x2, wx_x, wx_y, wx_z, wx;
	t_real 		pos_x, pos_y, pos_z;
	t_real       particle_mass[4];
...
// m_id_x = -1
				...
				wx = wx_x*wx_y*wx_z;
				particle_mass[0] += wx*local_mesh[l_ID];
				particle_mass[1] += wx*local_mesh[1715 + l_ID];
				particle_mass[2] += wx*local_mesh[3430 + l_ID];
				particle_mass[3] += wx*local_mesh[5145 + l_ID];
...
__kernel void kernel_collect_particles(
	...
	if(p_id < nparticle){
		...
		particle_mass[4*p_id]     = padded_particle_mass[						     cell_ID];
		particle_mass[4*p_id + 1] = padded_particle_mass[  max_np*ncell_int_padded + cell_ID];
		particle_mass[4*p_id + 2] = padded_particle_mass[2*max_np*ncell_int_padded + cell_ID];
		particle_mass[4*p_id + 3] = padded_particle_mass[3*max_np*ncell_int_padded + cell_ID];
	}
}
