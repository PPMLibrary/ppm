...
(
	...
	if((t_id[0] < mesh_size[0]) && (t_id[1] < mesh_size[1]) && (t_id[2] < mesh_size[2])){
		...
		mesh_padded[mesh_ID_padded +   nmesh_padded] = mesh[3*mesh_ID + 1];
		mesh_padded[mesh_ID_padded + 2*nmesh_padded] = mesh[3*mesh_ID + 2];
	}
}
...
(
	...
	if(p_id < nparticle){
		...
		particle_mass[3*p_id + 1] = padded_particle_mass[cell_ID +   max_np*ncell_int_padded];
		particle_mass[3*p_id + 2] = padded_particle_mass[cell_ID + 2*max_np*ncell_int_padded];
	}
}
