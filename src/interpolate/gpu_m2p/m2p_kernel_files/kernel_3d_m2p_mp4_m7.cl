...
__kernel void kernel_clist_duplicate_mesh(
	...
	t_id[1] = wg_id[1]*4  + local_id[1];
	t_id[2] = wg_id[2]*4  + local_id[2];
	...
	if((t_id[0] < mesh_size[0]) && (t_id[1] < mesh_size[1]) && (t_id[2] < mesh_size[2])){
		...
		mesh_ID_padded = (wg_id[0] + wg_id[1]*num_groups[0] + wg_id[2]*num_groups[0]*num_groups[1])*1715 + local_id[0] + local_id[1]*35 + local_id[2]*245;
...
(
	...
	if(p_id < nparticle) {
		...
		wg_y = (int)(cell_pos_y/4);
		wg_z = (int)(cell_pos_z/4);
...
__kernel void kernel_interpolate(
	...
	for(i = l_ID; i < 1715; i+= 512){
		local_mesh[i]	     = mesh_padded[                     wg_ID*1715 + i];
		local_mesh[1715 + i] = mesh_padded[  nmesh_ext_padded + wg_ID*1715 + i];
		local_mesh[3430 + i] = mesh_padded[2*nmesh_ext_padded + wg_ID*1715 + i];
		local_mesh[5145 + i] = mesh_padded[3*nmesh_ext_padded + wg_ID*1715 + i];
		local_mesh[6860 + i] = mesh_padded[4*nmesh_ext_padded + wg_ID*1715 + i];
		local_mesh[8575 + i] = mesh_padded[5*nmesh_ext_padded + wg_ID*1715 + i];
		local_mesh[10290 + i] = mesh_padded[6*nmesh_ext_padded + wg_ID*1715 + i];
	}
	barrier(CLK_LOCAL_MEM_FENCE);
	...
	for(rank = 0; rank < max_np; rank++){
		...
		pos_z  = particle_pos_data[(rank + 2*max_np)*ncell_int_padded + wg_ID*512 + l_ID];
		...
		l_ID = get_local_id(0) + get_local_id(1)*35 + get_local_id(2)*245;
...
// m_id_x = -1
					...
					particle_mass[1] += wx_x*wx_y*wx_z*local_mesh[1715 + l_ID];
					particle_mass[2] += wx_x*wx_y*wx_z*local_mesh[3430 + l_ID];
					particle_mass[3] += wx_x*wx_y*wx_z*local_mesh[5145 + l_ID];
					particle_mass[4] += wx_x*wx_y*wx_z*local_mesh[6860 + l_ID];
					particle_mass[5] += wx_x*wx_y*wx_z*local_mesh[8575 + l_ID];
					particle_mass[6] += wx_x*wx_y*wx_z*local_mesh[10290 + l_ID];
