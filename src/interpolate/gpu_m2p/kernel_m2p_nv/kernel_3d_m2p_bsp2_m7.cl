...
(
	...
	for(i = l_ID; i < (BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)*(BLOCK_SIZE_Z+1); i+= BLOCK_SIZE_X*BLOCK_SIZE_Y*BLOCK_SIZE_Z){
		...
		local_mesh[i + 6*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)*(BLOCK_SIZE_Z+1)] = mesh_padded[6*nmesh_ext_padded + wg_ID*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)*(BLOCK_SIZE_Z+1) + i];
	}
	barrier(CLK_LOCAL_MEM_FENCE);
	...
	for(rank = 0; rank < max_np; rank++){
		...
		for(m_id_z = 0; m_id_z < 2; m_id_z++){
			...
			for(m_id_y = 0; m_id_y < 2; m_id_y++){
				...
				for(m_id_x = 0; m_id_x < 2; m_id_x++){	
					...
					particle_mass[6] += wx_x*wx_y*wx_z*local_mesh[l_ID + 6*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)*(BLOCK_SIZE_Z+1)];
		...
		particle_mass_data[(rank + 6*max_np)*ncell_int_padded + wg_ID*BLOCK_SIZE_X*BLOCK_SIZE_Y*BLOCK_SIZE_Z + l_ID] = particle_mass[6];
	}
}
