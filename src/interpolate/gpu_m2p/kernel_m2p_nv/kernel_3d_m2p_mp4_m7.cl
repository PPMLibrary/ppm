...
__kernel void kernel_interpolate(
	...
	for(i = l_ID; i < (BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3)*(BLOCK_SIZE_Z+3); i+= BLOCK_SIZE_X*BLOCK_SIZE_Y){
		...
		local_mesh[3*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3)*(BLOCK_SIZE_Z+3) + i] = mesh_padded[3*nmesh_ext_padded + wg_ID*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3)*(BLOCK_SIZE_Z+3) + i];
		local_mesh[4*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3)*(BLOCK_SIZE_Z+3) + i] = mesh_padded[4*nmesh_ext_padded + wg_ID*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3)*(BLOCK_SIZE_Z+3) + i];
		local_mesh[5*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3)*(BLOCK_SIZE_Z+3) + i] = mesh_padded[5*nmesh_ext_padded + wg_ID*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3)*(BLOCK_SIZE_Z+3) + i];
		local_mesh[6*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3)*(BLOCK_SIZE_Z+3) + i] = mesh_padded[6*nmesh_ext_padded + wg_ID*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3)*(BLOCK_SIZE_Z+3) + i];
	}
	barrier(CLK_LOCAL_MEM_FENCE);
...
// m_id_x = -1
					...
					particle_mass[3] += wx_x*wx_y*wx_z*local_mesh[3*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3)*(BLOCK_SIZE_Z+3) + l_ID];
					particle_mass[4] += wx_x*wx_y*wx_z*local_mesh[4*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3)*(BLOCK_SIZE_Z+3) + l_ID];
					particle_mass[5] += wx_x*wx_y*wx_z*local_mesh[5*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3)*(BLOCK_SIZE_Z+3) + l_ID];
					particle_mass[6] += wx_x*wx_y*wx_z*local_mesh[6*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3)*(BLOCK_SIZE_Z+3) + l_ID];
