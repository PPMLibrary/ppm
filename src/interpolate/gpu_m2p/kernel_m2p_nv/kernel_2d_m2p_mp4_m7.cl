...
__kernel void kernel_clist_duplicate_mesh(
	...
	wg_ID = g_ID/((BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3));	
	l_ID  = g_ID - (BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3)*wg_ID;
	...
	if((t_id[0] < mesh_size[0]) && (t_id[1] < mesh_size[1])){
		...
		mesh_ID_padded = (wg_id[0] + wg_id[1]*num_groups[0])*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3) + local_id[0] + local_id[1]*(BLOCK_SIZE_X+3);

		mesh_padded[mesh_ID_padded] 			     = mesh[7*mesh_ID];
		mesh_padded[nmesh_padded + mesh_ID_padded]   = mesh[7*mesh_ID + 1];
		mesh_padded[2*nmesh_padded + mesh_ID_padded] = mesh[7*mesh_ID + 2];
		mesh_padded[3*nmesh_padded + mesh_ID_padded] = mesh[7*mesh_ID + 3];
		mesh_padded[4*nmesh_padded + mesh_ID_padded] = mesh[7*mesh_ID + 4];
		mesh_padded[5*nmesh_padded + mesh_ID_padded] = mesh[7*mesh_ID + 5];
		mesh_padded[6*nmesh_padded + mesh_ID_padded] = mesh[7*mesh_ID + 6];
	}
}
...
__kernel void kernel_interpolate(
	...
	for(i = l_ID; i < (BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3); i+= BLOCK_SIZE_X*BLOCK_SIZE_Y){
		local_mesh[       i] = mesh_padded[wg_ID*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3) + i];
		local_mesh[(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3)  + i] = mesh_padded[nmesh_ext_padded + wg_ID*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3) + i];
		local_mesh[2*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3) + i] = mesh_padded[2*nmesh_ext_padded + wg_ID*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3) + i];
		local_mesh[3*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3) + i] = mesh_padded[3*nmesh_ext_padded + wg_ID*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3) + i];
		local_mesh[4*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3) + i] = mesh_padded[4*nmesh_ext_padded + wg_ID*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3) + i];
		local_mesh[5*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3) + i] = mesh_padded[5*nmesh_ext_padded + wg_ID*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3) + i];
		local_mesh[6*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3) + i] = mesh_padded[6*nmesh_ext_padded + wg_ID*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3) + i];
	}
	barrier(CLK_LOCAL_MEM_FENCE);
...
// m_id.x = -1
			...
			particle_mass[1] += wx_x*wx_y*local_mesh[(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3)  + l_ID];
			particle_mass[2] += wx_x*wx_y*local_mesh[2*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3) + l_ID];
			particle_mass[3] += wx_x*wx_y*local_mesh[3*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3) + l_ID];
			particle_mass[4] += wx_x*wx_y*local_mesh[4*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3) + l_ID];
			particle_mass[5] += wx_x*wx_y*local_mesh[5*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3) + l_ID];
			particle_mass[6] += wx_x*wx_y*local_mesh[6*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3) + l_ID];
...
// m_id.x = 2
		...
		particle_mass_data[(rank+3*max_np)*ncell_int_padded + wg_ID*BLOCK_SIZE_X*BLOCK_SIZE_Y + l_ID] = particle_mass[3]; 
		particle_mass_data[(rank+4*max_np)*ncell_int_padded + wg_ID*BLOCK_SIZE_X*BLOCK_SIZE_Y + l_ID] = particle_mass[4]; 
		particle_mass_data[(rank+5*max_np)*ncell_int_padded + wg_ID*BLOCK_SIZE_X*BLOCK_SIZE_Y + l_ID] = particle_mass[5]; 
		particle_mass_data[(rank+6*max_np)*ncell_int_padded + wg_ID*BLOCK_SIZE_X*BLOCK_SIZE_Y + l_ID] = particle_mass[6]; 
	}
}
...
__kernel void kernel_collect_particles(
	...
	if(p_id < nparticle){
		...
		particle_mass[7*p_id + 1] = padded_particle_mass[max_np*ncell_int_padded + cell_ID];
		particle_mass[7*p_id + 2] = padded_particle_mass[2*max_np*ncell_int_padded + cell_ID];
		particle_mass[7*p_id + 3] = padded_particle_mass[3*max_np*ncell_int_padded + cell_ID];
		particle_mass[7*p_id + 4] = padded_particle_mass[4*max_np*ncell_int_padded + cell_ID];
		particle_mass[7*p_id + 5] = padded_particle_mass[5*max_np*ncell_int_padded + cell_ID];
		particle_mass[7*p_id + 6] = padded_particle_mass[6*max_np*ncell_int_padded + cell_ID];
	}
}
