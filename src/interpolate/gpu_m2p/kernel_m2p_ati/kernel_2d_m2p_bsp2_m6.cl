...
(
	...
	if((t_id[0] < mesh_size[0]) && (t_id[1] < mesh_size[1])){
		...
		mesh_padded[mesh_ID_padded +   nmesh_padded] = mesh[6*mesh_ID + 1];
		mesh_padded[mesh_ID_padded + 2*nmesh_padded] = mesh[6*mesh_ID + 2];
		mesh_padded[mesh_ID_padded + 3*nmesh_padded] = mesh[6*mesh_ID + 3];
		mesh_padded[mesh_ID_padded + 4*nmesh_padded] = mesh[6*mesh_ID + 4];
		mesh_padded[mesh_ID_padded + 5*nmesh_padded] = mesh[6*mesh_ID + 5];
	}
}
...
(
	...
	for(i = l_ID; i < (BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1); i+= BLOCK_SIZE_X*BLOCK_SIZE_Y){
		...
		local_mesh[i + (BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)]  = mesh_padded[wg_ID*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1) + i +   nmesh_ext_padded];
		local_mesh[i + 2*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)] = mesh_padded[wg_ID*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1) + i + 2*nmesh_ext_padded];
		local_mesh[i + 3*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)] = mesh_padded[wg_ID*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1) + i + 3*nmesh_ext_padded];
		local_mesh[i + 4*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)] = mesh_padded[wg_ID*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1) + i + 4*nmesh_ext_padded];
		local_mesh[i + 5*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)] = mesh_padded[wg_ID*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1) + i + 5*nmesh_ext_padded];
	}
	barrier(CLK_LOCAL_MEM_FENCE);
	...
	for(rank = 0; rank < max_np; rank++){
		...
		for(m_id.y = 0; m_id.y < 2; m_id.y++){
			...
			for(m_id.x = 0; m_id.x < 2; m_id.x++){	
				...
				particle_mass[1] += wx_x*wx_y*local_mesh[l_ID + (BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)];
				particle_mass[2] += wx_x*wx_y*local_mesh[l_ID + 2*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)];
				particle_mass[3] += wx_x*wx_y*local_mesh[l_ID + 3*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)];
				particle_mass[4] += wx_x*wx_y*local_mesh[l_ID + 4*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)];
				particle_mass[5] += wx_x*wx_y*local_mesh[l_ID + 5*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)];
...
(
	...
	if(p_id < nparticle){
		...
		particle_mass[6*p_id + 1] = padded_particle_mass[cell_ID +   max_np*ncell_int_padded];
		particle_mass[6*p_id + 2] = padded_particle_mass[cell_ID + 2*max_np*ncell_int_padded];
		particle_mass[6*p_id + 3] = padded_particle_mass[cell_ID + 3*max_np*ncell_int_padded];
		particle_mass[6*p_id + 4] = padded_particle_mass[cell_ID + 4*max_np*ncell_int_padded];
		particle_mass[6*p_id + 5] = padded_particle_mass[cell_ID + 5*max_np*ncell_int_padded];
	}
}
