...
__kernel void kernel_clist_duplicate_mesh(
	...
	if((t_id[0] < mesh_size[0]) && (t_id[1] < mesh_size[1])){
		...
		mesh_padded[mesh_ID_padded] 			     = mesh[6*mesh_ID];
		mesh_padded[nmesh_padded + mesh_ID_padded]   = mesh[6*mesh_ID + 1];
		mesh_padded[2*nmesh_padded + mesh_ID_padded] = mesh[6*mesh_ID + 2];
		mesh_padded[3*nmesh_padded + mesh_ID_padded] = mesh[6*mesh_ID + 3];
		mesh_padded[4*nmesh_padded + mesh_ID_padded] = mesh[6*mesh_ID + 4];
		mesh_padded[5*nmesh_padded + mesh_ID_padded] = mesh[6*mesh_ID + 5];
	}
}
...
__kernel void kernel_collect_particles(
	...
	if(p_id < nparticle){
		...
		particle_mass[6*p_id]	  = padded_particle_mass[cell_ID];
		particle_mass[6*p_id + 1] = padded_particle_mass[max_np*ncell_int_padded + cell_ID];
		particle_mass[6*p_id + 2] = padded_particle_mass[2*max_np*ncell_int_padded + cell_ID];
		particle_mass[6*p_id + 3] = padded_particle_mass[3*max_np*ncell_int_padded + cell_ID];
		particle_mass[6*p_id + 4] = padded_particle_mass[4*max_np*ncell_int_padded + cell_ID];
		particle_mass[6*p_id + 5] = padded_particle_mass[5*max_np*ncell_int_padded + cell_ID];
	}
}
