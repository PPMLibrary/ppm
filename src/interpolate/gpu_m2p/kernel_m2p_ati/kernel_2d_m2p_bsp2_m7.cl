...
(
	...
	for(i = l_ID; i < (BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1); i+= BLOCK_SIZE_X*BLOCK_SIZE_Y){
		...
		local_mesh[i + 6*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)] = mesh_padded[wg_ID*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1) + i + 6*nmesh_ext_padded];
	}
	barrier(CLK_LOCAL_MEM_FENCE);
	...
	for(rank = 0; rank < max_np; rank++){
		...
		for(m_id.y = 0; m_id.y < 2; m_id.y++){
			...
			for(m_id.x = 0; m_id.x < 2; m_id.x++){	
				...
				particle_mass[6] += wx_x*wx_y*local_mesh[l_ID + 6*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)];
