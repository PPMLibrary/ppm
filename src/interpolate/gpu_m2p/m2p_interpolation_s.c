...
(
					int*   mesh_size,
          t_real* minphys_with_ghost,
          t_real* maxphys_with_ghost,
          int*   np,
					int*   ndim_in,
					int*   nmass_in,
					int*   interp_kernel
	...
	if((ndim_prev   != ndim)  ||
       ...
       (last_kernel != *interp_kernel))
	   {
    	...
    	for (dim = 0; dim < ndim; dim++){
        	...
        	h_s[dim]  = (maxphys_with_ghost[dim] - minphys_with_ghost[dim])/(mesh_size_ext[dim] - 1);
    		// extend input domain including ghost layers
       		maxphys_s[dim]  = maxphys_with_ghost[dim] - h_s[dim]*kernel_size[dim];
        	minphys_s[dim]  = minphys_with_ghost[dim] + h_s[dim]*kernel_size[dim];
    	...
    	dev_mass     = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(cl_t_real)*nparticle*nmass, NULL, &error);
		error_buffer |= error;
    	...
    	dev_mesh_padded = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(cl_t_real)*nmesh_ext_padded*nmass, NULL, &error);
		error_buffer |= error;
...
(
    ...
    error |= clSetKernelArg(kernel, n++, sizeof(cl_mem), (void *)&dev_mesh_padded);
    error |= clSetKernelArg(kernel, n++, sizeof(cl_t_real)*(1715*nmass), NULL);
