...
// m_id_x = -1
				...
				local_mesh[l_ID + 665]  += wx_x*wx_y*p_mass[1];
				local_mesh[l_ID + 1330] += wx_x*wx_y*p_mass[2];
				local_mesh[l_ID + 1995] += wx_x*wx_y*p_mass[3];
				local_mesh[l_ID + 2660] += wx_x*wx_y*p_mass[4];
				local_mesh[l_ID + 3325] += wx_x*wx_y*p_mass[5];
				local_mesh[l_ID + 3990] += wx_x*wx_y*p_mass[6];
				barrier(CLK_LOCAL_MEM_FENCE);
...
// m_id_x = 2
	...
	for(i = l_ID; i < 665; i+=512){
		...
		mesh_re[  nmesh_ext_padded + wg_ID*665 + i] = local_mesh[i + 665];
		mesh_re[2*nmesh_ext_padded + wg_ID*665 + i] = local_mesh[i + 1330];
		mesh_re[3*nmesh_ext_padded + wg_ID*665 + i] = local_mesh[i + 1995];
		mesh_re[4*nmesh_ext_padded + wg_ID*665 + i] = local_mesh[i + 2660];
		mesh_re[5*nmesh_ext_padded + wg_ID*665 + i] = local_mesh[i + 3325];
		mesh_re[6*nmesh_ext_padded + wg_ID*665 + i] = local_mesh[i + 3990];
	}
}
