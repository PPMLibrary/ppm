...
// m_id_x = -1
				...
				local_mesh[l_ID + 1715] += wx_x*wx_y*wx_z*p_mass[1];
				local_mesh[l_ID + 3430] += wx_x*wx_y*wx_z*p_mass[2];
				local_mesh[l_ID + 5145] += wx_x*wx_y*wx_z*p_mass[3];
				local_mesh[l_ID + 6860] += wx_x*wx_y*wx_z*p_mass[4];
				local_mesh[l_ID + 8575] += wx_x*wx_y*wx_z*p_mass[5];
				local_mesh[l_ID + 10290] += wx_x*wx_y*wx_z*p_mass[6];
				barrier(CLK_LOCAL_MEM_FENCE);
...
// m_id_x = 2
			...
			l_ID += 31; // Move on to next mesh point in y-dimension
...
// m_id_x = 2
		...
		l_ID += 105; // Move on to next mesh point in z-dimension
		}
	...
	for(i = l_ID; i < 1715; i+=512){
		mesh_re[				     wg_ID*1715 + i] = local_mesh[i];
		mesh_re[nmesh_ext_padded   + wg_ID*1715 + i] = local_mesh[i + 1715];
		mesh_re[2*nmesh_ext_padded + wg_ID*1715 + i] = local_mesh[i + 3430];
		mesh_re[3*nmesh_ext_padded + wg_ID*1715 + i] = local_mesh[i + 5145];
		mesh_re[4*nmesh_ext_padded + wg_ID*1715 + i] = local_mesh[i + 6860];
		mesh_re[5*nmesh_ext_padded + wg_ID*1715 + i] = local_mesh[i + 8575];
		mesh_re[6*nmesh_ext_padded + wg_ID*1715 + i] = local_mesh[i + 10290];
	}
}
...
void get_mesh_ID_wg_padded(
	...
	*wg_id_z = global_id_z/4;
	...
	global_id_y = 3 + local_id_y + (*wg_id_y)*7;
	global_id_z = 3 + local_id_z + (*wg_id_z)*7;

	local_id_x = global_id_x  - (*wg_id_x)*35; 
	local_id_y = global_id_y  - (*wg_id_y)*7; 
	local_id_z = global_id_z  - (*wg_id_z)*7; 
	local_ID   = local_id_x + local_id_y*35 + local_id_z*245;
	*mesh_ID_padded = wg_ID*1715 + local_ID;
}
