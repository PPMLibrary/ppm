...
(
		...
		for(m_id_z = 0; m_id_z < 2; m_id_z++){
			...
			for(m_id_y = 0; m_id_y < 2; m_id_y++){
				...
				for(m_id_x = 0; m_id_x < 2; m_id_x++){	
					...
					local_mesh[l_ID + 3300] += wx_x*wx_y*wx_z*p_mass[4];
					local_mesh[l_ID + 4125] += wx_x*wx_y*wx_z*p_mass[5];
					local_mesh[l_ID + 4950] += wx_x*wx_y*wx_z*p_mass[6];
					barrier(CLK_LOCAL_MEM_FENCE);
	...
	for(i = l_ID; i < 825; i+=512){
		...
		mesh_re[wg_ID*825 + i + 4*nmesh_ext_padded] = local_mesh[i + 3300];
		mesh_re[wg_ID*825 + i + 5*nmesh_ext_padded] = local_mesh[i + 4125];
		mesh_re[wg_ID*825 + i + 6*nmesh_ext_padded] = local_mesh[i + 4950];
	}
}
