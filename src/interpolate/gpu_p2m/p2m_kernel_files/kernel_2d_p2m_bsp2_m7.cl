...
(
	...
	for(rank = 0; rank < max_np; rank++){
		...
		p_mass[4] = particle_mass_data[(rank + 4*max_np)*ncell_ext_padded + wg_ID*512 + l_ID];
		p_mass[5] = particle_mass_data[(rank + 5*max_np)*ncell_ext_padded + wg_ID*512 + l_ID];
		p_mass[6] = particle_mass_data[(rank + 6*max_np)*ncell_ext_padded + wg_ID*512 + l_ID];
		...
		for(m_id.y = 0; m_id.y < 2; m_id.y++){
			...
			for(m_id.x = 0; m_id.x < 2; m_id.x++){	
				...
				local_mesh[l_ID + 561]  += wx_x*wx_y*p_mass[1];
				local_mesh[l_ID + 1122] += wx_x*wx_y*p_mass[2];
				local_mesh[l_ID + 1683] += wx_x*wx_y*p_mass[3];
				local_mesh[l_ID + 2244] += wx_x*wx_y*p_mass[4];
				local_mesh[l_ID + 2805] += wx_x*wx_y*p_mass[5];
				local_mesh[l_ID + 3366] += wx_x*wx_y*p_mass[6];
				barrier(CLK_LOCAL_MEM_FENCE);
	...
	for(i = l_ID; i < 561; i+=512){
		mesh_re[wg_ID*561 + i] 						= local_mesh[i];
		mesh_re[wg_ID*561 + i +   nmesh_ext_padded] = local_mesh[i + 561];
		mesh_re[wg_ID*561 + i + 2*nmesh_ext_padded] = local_mesh[i + 1122];
		mesh_re[wg_ID*561 + i + 3*nmesh_ext_padded] = local_mesh[i + 1683];
		mesh_re[wg_ID*561 + i + 4*nmesh_ext_padded] = local_mesh[i + 2244];
		mesh_re[wg_ID*561 + i + 5*nmesh_ext_padded] = local_mesh[i + 2805];
		mesh_re[wg_ID*561 + i + 6*nmesh_ext_padded] = local_mesh[i + 3366];
	}
}
...
(
	...
	*wg_id_y = global_id_y/16;
	...
	global_id_y = 1 + local_id_y + (*wg_id_y)*17;

	local_id_x = global_id_x  - (*wg_id_x)*33; 
	local_id_y = global_id_y  - (*wg_id_y)*17; 
	local_ID   = local_id_x + local_id_y*33;
	*mesh_ID_padded = wg_ID*561 + local_ID;
}
