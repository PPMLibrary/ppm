...
(
	...
	for(i = l_ID; i < 7*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)*(BLOCK_SIZE_Z+1); i+=BLOCK_SIZE_X*BLOCK_SIZE_Y*BLOCK_SIZE_Z){
		local_mesh[i] = 0.0;
	}
	barrier(CLK_LOCAL_MEM_FENCE);
	...
	for(rank = 0; rank < max_np; rank++){
		...
		p_mass[4] = particle_mass_data[(rank + 4*max_np)*ncell_ext_padded + wg_ID*BLOCK_SIZE_X*BLOCK_SIZE_Y*BLOCK_SIZE_Z + l_ID];
		p_mass[5] = particle_mass_data[(rank + 5*max_np)*ncell_ext_padded + wg_ID*BLOCK_SIZE_X*BLOCK_SIZE_Y*BLOCK_SIZE_Z + l_ID];
		p_mass[6] = particle_mass_data[(rank + 6*max_np)*ncell_ext_padded + wg_ID*BLOCK_SIZE_X*BLOCK_SIZE_Y*BLOCK_SIZE_Z + l_ID];
		...
		for(m_id_z = 0; m_id_z < 2; m_id_z++){
			...
			for(m_id_y = 0; m_id_y < 2; m_id_y++){
				...
				for(m_id_x = 0; m_id_x < 2; m_id_x++){	
					...
					local_mesh[l_ID + (BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)*(BLOCK_SIZE_Z+1)]  += wx_x*wx_y*wx_z*p_mass[1];
					local_mesh[l_ID + 2*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)*(BLOCK_SIZE_Z+1)] += wx_x*wx_y*wx_z*p_mass[2];
					local_mesh[l_ID + 3*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)*(BLOCK_SIZE_Z+1)] += wx_x*wx_y*wx_z*p_mass[3];
					local_mesh[l_ID + 4*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)*(BLOCK_SIZE_Z+1)] += wx_x*wx_y*wx_z*p_mass[4];
					local_mesh[l_ID + 5*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)*(BLOCK_SIZE_Z+1)] += wx_x*wx_y*wx_z*p_mass[5];
					local_mesh[l_ID + 6*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)*(BLOCK_SIZE_Z+1)] += wx_x*wx_y*wx_z*p_mass[6];
					barrier(CLK_LOCAL_MEM_FENCE);
	...
	for(i = l_ID; i < (BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)*(BLOCK_SIZE_Z+1); i+=BLOCK_SIZE_X*BLOCK_SIZE_Y*BLOCK_SIZE_Z){
		mesh_re[wg_ID*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)*(BLOCK_SIZE_Z+1) + i] 				   	    = local_mesh[i];
		mesh_re[wg_ID*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)*(BLOCK_SIZE_Z+1) + i +   nmesh_ext_padded] = local_mesh[i + (BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)*(BLOCK_SIZE_Z+1)];
		mesh_re[wg_ID*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)*(BLOCK_SIZE_Z+1) + i + 2*nmesh_ext_padded] = local_mesh[i + 2*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)*(BLOCK_SIZE_Z+1)];
		mesh_re[wg_ID*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)*(BLOCK_SIZE_Z+1) + i + 3*nmesh_ext_padded] = local_mesh[i + 3*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)*(BLOCK_SIZE_Z+1)];
		mesh_re[wg_ID*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)*(BLOCK_SIZE_Z+1) + i + 4*nmesh_ext_padded] = local_mesh[i + 4*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)*(BLOCK_SIZE_Z+1)];
		mesh_re[wg_ID*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)*(BLOCK_SIZE_Z+1) + i + 5*nmesh_ext_padded] = local_mesh[i + 5*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)*(BLOCK_SIZE_Z+1)];
		mesh_re[wg_ID*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)*(BLOCK_SIZE_Z+1) + i + 6*nmesh_ext_padded] = local_mesh[i + 6*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)*(BLOCK_SIZE_Z+1)];
	}
}
...
void get_mesh_ID_wg_padded(
	...
	*wg_id_z = global_id_z/BLOCK_SIZE_Z;
	...
	global_id_z = 1 + local_id_z + (*wg_id_z)*(BLOCK_SIZE_Z+1);

	local_id_x = global_id_x  - (*wg_id_x)*(BLOCK_SIZE_X+1); 
	local_id_y = global_id_y  - (*wg_id_y)*(BLOCK_SIZE_Y+1); 
	local_id_z = global_id_z  - (*wg_id_z)*(BLOCK_SIZE_Z+1); 
	local_ID   = local_id_x + local_id_y*(BLOCK_SIZE_X+1) + local_id_z*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1);
	*mesh_ID_padded = wg_ID*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)*(BLOCK_SIZE_Z+1) + local_ID;
}
