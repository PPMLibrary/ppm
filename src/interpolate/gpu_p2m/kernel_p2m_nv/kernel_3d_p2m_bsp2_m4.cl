...
(
	...
	if(p_id < nparticle) {
		...
		cell_ID    = cell_pos_x + BLOCK_SIZE_X*cell_pos_y + BLOCK_SIZE_X*BLOCK_SIZE_Y*cell_pos_z + (wg_x + num_groups[0]*wg_y + num_groups[0]*num_groups[1]*wg_z)*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1)*(BLOCK_SIZE_Z+1); 
...
(
	...
	for(i = l_ID; i < (BLOCK_SIZE_X+1)00; i+=BLOCK_SIZE_X*BLOCK_SIZE_Y*BLOCK_SIZE_Z){
		local_mesh[i] = 0.0;
	}
	barrier(CLK_LOCAL_MEM_FENCE);
	...
	for(rank = 0; rank < max_np; rank++){
		...
		l_ID = get_local_id(0) + get_local_id(1)*(BLOCK_SIZE_X+1) + get_local_id(2)*165;
...
void get_mesh_ID_wg_padded(
	...
	*wg_id_z = global_id_z/BLOCK_SIZE_Z;
	...
	local_ID   = local_id_x + local_id_y*(BLOCK_SIZE_X+1) + local_id_z*165;
