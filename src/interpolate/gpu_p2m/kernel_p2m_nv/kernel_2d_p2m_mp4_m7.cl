...
__kernel void kernel_interpolate(
	...
	for(i = l_ID; i < 7*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3); i+=BLOCK_SIZE_X*BLOCK_SIZE_Y){
		local_mesh[i] = 0.0;
	}
	barrier(CLK_LOCAL_MEM_FENCE);
...
// m_id_x = -1
				...
				local_mesh[l_ID + 3*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3)] += wx_x*wx_y*p_mass[3];
				local_mesh[l_ID + 4*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3)] += wx_x*wx_y*p_mass[4];
				local_mesh[l_ID + 5*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3)] += wx_x*wx_y*p_mass[5];
				local_mesh[l_ID + 6*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3)] += wx_x*wx_y*p_mass[6];
				barrier(CLK_LOCAL_MEM_FENCE);
...
// m_id_x = 2
	...
	for(i = l_ID; i < (BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3); i+=BLOCK_SIZE_X*BLOCK_SIZE_Y){
		...
		mesh_re[3*nmesh_ext_padded + wg_ID*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3) + i] = local_mesh[i + 3*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3)];
		mesh_re[4*nmesh_ext_padded + wg_ID*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3) + i] = local_mesh[i + 4*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3)];
		mesh_re[5*nmesh_ext_padded + wg_ID*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3) + i] = local_mesh[i + 5*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3)];
		mesh_re[6*nmesh_ext_padded + wg_ID*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3) + i] = local_mesh[i + 6*(BLOCK_SIZE_X+3)*(BLOCK_SIZE_Y+3)];
	}
}
