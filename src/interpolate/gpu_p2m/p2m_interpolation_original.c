...
int ppm_gpu_p2m_initialize_(
	...
	if((ndim_prev   != ndim)  ||
       ...
       (last_kernel != *interp_kernel))
	   {
    	...
    	clBuild(filename, 1, &program, &context, &device);	
	...
	if((ndim_prev   != ndim)  ||
       ...
       (last_kernel != *interp_kernel))
	   {
    	...
    	for(dim = 0; dim < ndim; dim++){
        	...
        	h[dim]  = (maxphys_wo_ghost[dim] - minphys_wo_ghost[dim])/(mesh_size[dim] - 1);
    		// extend input domain including ghost layers
        	maxphys[dim]  = maxphys_wo_ghost[dim] + h[dim]*kernel_size[dim];
        	minphys[dim]  = minphys_wo_ghost[dim] - h[dim]*kernel_size[dim];
		...
		dev_minphys = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(cl_float)*ndim, minphys, &error);
    	// Buffer 4: Coordinates of maximum extent of physical domain
		dev_maxphys = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(cl_float)*ndim, maxphys, &error);
    	// Buffer 5: h values
		dev_h = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(cl_float)*ndim, h, &error);
    	...
    	dev_mesh_size = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(cl_float)*ndim, mesh_size, &error);
    	// Buffer 13: Mesh sizes when padded
    	dev_mesh_size_padded = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(cl_float)*ndim, mesh_size_padded, &error);
    	// Buffer 14: Mesh points
    	dev_mesh = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(cl_float)*nmesh*nmass, NULL, &error);
    	// Buffer 15: Mesh points, padded
    	dev_mesh_padded = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(cl_float)*nmesh_padded*nmass, NULL, &error);
		...
		last_call    = p2m_interpolation;
		last_kernel  = *interp_kernel;
		ptr2clean_up = &ppm_gpu_p2m_clean_up;
	}
...
int ppm_gpu_p2m_interpolation_(
	...
	float t_elapsed, t_elapsed_total;

    // This call will run initialization routines if necessary
	ppm_gpu_p2m_initialize_(mesh_size, minphys_wo_ghost, maxphys_wo_ghost, np, ndim_in, nmass_in, interp_kernel);
	
// CREATE BUFFERS //
    // Buffer 1: Positions of particles
    dev_xp_const 		= clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(cl_float)*nparticle*ndim, NULL, &error);
    // Buffer 2: Mass values of particles
    dev_mass_const = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(cl_float)*nparticle*nmass, NULL, &error);

	error = clEnqueueWriteBuffer(queue, dev_mass_const, CL_FALSE, 0, sizeof(cl_float)*nparticle*nmass, mass, 0, NULL, &event_write_mass);
    checkError(error, "clEnqueueWriteBuffer while writing dev_mass_const", __FUNCTION__, __LINE__, stamp);
	error = clEnqueueWriteBuffer(queue, dev_xp_const, CL_FALSE, 0, sizeof(cl_float)*nparticle*ndim, xp, 0, NULL, &event_write_xp);
    checkError(error, "clEnqueueWriteBuffer while writing dev_xp_const", __FUNCTION__, __LINE__, stamp);

// INITIALIZING TO ZERO //
	error = p2m_initialize_np_cell();
...
// GET CELL INDICES OF PARTICLES AND COUNT NUMBER OF PARTICLES IN CELLS //
	error = p2m_get_particle_positions(nparticle, ndim, nmass);
...
// READ BACK RESULTS //
    ...
    error = clEnqueueReadBuffer(queue, dev_mesh, CL_FALSE, 0, sizeof(float)*nmesh*nmass, mesh, 0, NULL, &event_read_mesh);
...
int p2m_reduce_np_cell()
{
	int 	  n, error;
	size_t 	  localWorkSize, globalWorkSize, num_wg;
	cl_kernel ck_init_max_np, ck_init_max_np2, ck_get_max_np, ck_get_max_np2; 
	size_t 	  size_max_np, size_max_np2;
...
(
	...
	size_sorted_particle_position = sizeof(float)*n_sorted_particle_position;
	
	n_sorted_particle_mass = max_np*nmass*ncell_ext_padded + 1536; // Extra padding in order to avoid if branch
	size_sorted_particle_mass = sizeof(float)*n_sorted_particle_mass;
...
int	init_sorted_particle_mass(){
    ...
    ck_init_sorted_particle_mass = clCreateKernel(program, "kernel_clist_init_to_zero_float", &error);
...
int init_padded_mesh(int nmass)
{
    ...
    ck_init_padded_mesh = clCreateKernel(program, "kernel_clist_init_to_zero_float", &error);
...
(
    ...
    error |= clSetKernelArg(ck_interpolate, n++, sizeof(cl_float)*interp_lmem_size, NULL);
