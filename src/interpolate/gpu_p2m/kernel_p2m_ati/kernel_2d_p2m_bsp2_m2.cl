...
(
	...
	for(i = l_ID; i < 2*(BLOCK_SIZE_X+1)*(BLOCK_SIZE_Y+1); i+=BLOCK_SIZE_X*BLOCK_SIZE_Y){
		local_mesh[i] = 0.0;
	}
	barrier(CLK_LOCAL_MEM_FENCE);
...
(
	...
	if((mesh_id_x < mesh_size[0]) && (mesh_id_y < mesh_size[1])) {
		...
		mesh[2*mesh_ID] 		= total_value[0];
		mesh[2*mesh_ID + 1] 	= total_value[1];
	}
}
