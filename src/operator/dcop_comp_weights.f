      ...
      SUBROUTINE DTYPE(dcop_comp_weights_3d)(this,info,c,min_sv)
          ...
          c_value=MERGE(c,1._MK,PRESENT(c))
          ...
          ldc(1)   =MERGE(Nlist%nneighmax*nterms,Nlist%nneighmax,vector)
          np_target=MERGE(Part_to%Mpart,Part_to%Npart,with_ghosts)
          ...
          DO i=1,nterms
             ...
             IF (.NOT.isinterp) THEN
                IF (SUM(alpha(1:ppm_dim,1)).EQ.0.AND.MOD(sum_degree(i),2).EQ.0) THEN
                   b_0(1,i) = 5._MK
                ENDIF
             ENDIF
          ENDDO
             ...
             IF (isinterp) THEN
                DO i=1,nterms
                   ...
                   DO ineigh = 1,nvlist(ip)
                      ...
                      eta(ineigh,ip)=DTYPE(primitive)(SQRT(dist2/nn_sq(iq)) / 0.9_MK)

                      !reuse the variable to assemble the rhs
                      DO j=1,ncoeff
                         b(j,i) = b(j,i)- eta(ineigh,ip)* &
#if   __DIM == 2
