...
!      a diagonal matrix Hk0, using information from the previous M step
...
!                                                                       
!     F       is a DOUBLE PRECISION variable. Before initial entry and o
...
!             to the subroutine. A return with IFLAG<0 indicates an erro
!             and IFLAG=0 indicates that the routine has terminated with
...
!                                                                       
!                                                                       
!              IFLAG=-2  The i-th diagonal element of the diagonal inver
...
!                                                                       
   80 ITER = ITER + 1 
      INFO = 0 
      BOUND = ITER - 1 
      IF (ITER.EQ.1) GOTO 165 
      IF (ITER.GT.M) BOUND = M 
!                                                                       
      YS = DDOT (N, W (IYPT + NPT + 1), 1, W (ISPT + NPT + 1), 1) 
      IF (.NOT.DIAGCO) THEN 
         YY = DDOT (N, W (IYPT + NPT + 1), 1, W (IYPT + NPT + 1),       &
         1)                                                             
         DO 90 I = 1, N 
   90    DIAG (I) = YS / YY 
      ELSE 
  ...
  100 CONTINUE 
      IF (DIAGCO) THEN 
         DO 110 I = 1, N 
  110    IF (DIAG (I) .LE.ZERO) GOTO 195 
      ENDIF 
!                                                                       
...
!                                                                       
!                                                                       
      SUBROUTINE LB1 (IPRINT, ITER, NFUN, GNORM, N, M, X, F, G, STP,    &
      FINISH)                                                           
!                                                                       
!     -------------------------------------------------------------     
...
!                                                                       
      IF (ITER.EQ.0) THEN 
         ...
         WRITE (MP, 70) 
      ELSE 
         IF ( (IPRINT (1) .EQ.0) .AND. (ITER.NE.1.AND..NOT.FINISH) )    &
         RETURN                                                         
...
!     **********                                                        
      ...
      FINIT, FTEST1, FM, FX, FXM, FY, FYM, P5, P66, STX, STY, STMIN,    &
...
!                                                                       
      IF (N.LE.0.OR.STP.LE.ZERO.OR.FTOL.LT.ZERO.OR.GTOL.LT.ZERO.OR.XTOL.&
     &LT.ZERO.OR.STPMIN.LT.ZERO.OR.STPMAX.LT.STPMIN.OR.MAXFEV.LE.0) RETU&
     &RN                                                                
!                                                                       
...
!                                                                       
      ...
      .OR.NFEV.GE.MAXFEV - 1.OR.INFOC.EQ.0.OR. (BRACKT.AND.STMAX -      &
      STMIN.LE.XTOL * STMAX) ) STP = STX                                
!                                                                       
...
!                                                                       
      IF ( (BRACKT.AND. (STP.LE.STMIN.OR.STP.GE.STMAX) ) .OR.INFOC.EQ.0)&
      INFO = 6                                                          
...
!                                                                       
      IF (STAGE1.AND.F.LE.FTEST1.AND.DG.GE.MIN (FTOL, GTOL) * DGINIT)   &
      STAGE1 = .FALSE.                                                  
!                                                                       
...
!     CHECK THE INPUT PARAMETERS FOR ERRORS.                            
!                                                                       
      IF ( (BRACKT.AND. (STP.LE.MIN (STX, STY) .OR.STP.GE.MAX (STX, STY)&
      ) ) .OR.DX * (STP - STX) .GE.0.0.OR.STPMAX.LT.STPMIN) RETURN      
!                                                                       
...
!                                                                       
         GAMMA = S * SQRT (MAX (0.0D0, (THETA / S) **2 - (DX / S)       &
         * (DP / S) ) )                                                 
         IF (STP.GT.STX) GAMMA = - GAMMA 
         P = (GAMMA - DP) + THETA 
         Q = (GAMMA + (DX - DP) ) + GAMMA 
