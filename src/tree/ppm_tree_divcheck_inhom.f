     ...
     &    fixed,boxcost,neigh_constraints,num_constr,ndiv,info)
      ...
      !!! Minimum box sizes for each box and dimension
      !!!
      !!! 1st index: x,y[,z]                                                   +
      !!! 2nd: box ID
...
!                      print *, 'first ', i, k, temp_r
!                   ENDIF
               IF (result_array(j,k,1)+lmyeps .LT. min_box(j,i)+minboxsizes(j,i)) THEN
               
                  ! Find the first constraint at the minboxsize position
                  temp_r = k
                  DO WHILE (.NOT.((min_box(j,i)+minboxsizes(j,i)-lmyeps).GT.result_array(j,k,1) &
            &                         .AND. (min_box(j,i)+minboxsizes(j,i)+lmyeps).LT.result_array(j,k,2)))
...
!                   ENDIF
            ...
            &                      .OR. result_array(j,k,2)+lmyeps .LT. min_box(j,i)+minboxsizes(j,i))
                           IF (result_array(j,k,2) .GT. result_array(j,temp_r,2)) THEN
                              temp_r = k
                           ENDIF
                     ...
                     ENDIF 
!                      IF (ppm_rank .EQ. 0)THEN
!                      print *, 'last ', i, k, temp_r, result_array(j,temp_r,2),max_box(j,i)-minboxsizes(j,i)
!                   ENDIF
   !                IF (result_array(j,k,1)+lmyeps .LT. min_box(j,i)+minboxsizes(j,i)) THEN
   !                   !     until distance is >= 0
   ...
   !                   ENDIF
                     ...
                     IF (result_array(j,temp_r,2)-lmyeps .GT. max_box(j,i)-minboxsizes(j,i)) THEN
                        is_possible(j) = .FALSE.
   !                      IF (ppm_rank .EQ. 0)THEN
 ...
 9999 CONTINUE
     ...
     &          'Number of boxes must be >= 0',__LINE__,info)
         ...
         DO i=1,ppm_dim
            DO j=1,nbox
               IF (minboxsizes(i,j) .LT. 0.0_MK) THEN
                  info = ppm_error_error
                  CALL ppm_error(ppm_err_argument,'ppm_tree_divcheck',     &
...
#elif __KIND == __DOUBLE_PRECISION
      END SUBROUTINE ppm_tree_divcheck_inhom_d
#endif
!<<<< haeckic end >>>>!