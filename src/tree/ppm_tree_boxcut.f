      ...
      REAL(ppm_kind_double), DIMENSION(:  ), INTENT(IN   ) :: cutpos
      !!! Positions of the cuts along the given axes.
      REAL(ppm_kind_double), DIMENSION(:,:), POINTER       :: mincut
      !!! Minimum coordinates of the cut boxes.
      !!!
      !!! 1st: x,y[,z]                                                         +
      !!! 2nd: ibox.
      REAL(ppm_kind_double), DIMENSION(:,:), POINTER       :: maxcut
      ...
      REAL(MK), DIMENSION(ppm_dim) :: mincut_
      REAL(MK), DIMENSION(ppm_dim) :: maxcut_
      ...
      !-------------------------------------------------------------------------
      ELSE IF (ncut.GT.3) THEN
         fail("Unknown cutting mode: ncut must be < 4.",ppm_error=ppm_error_fatal)
      ENDIF
      ...
      IF (have_particles) THEN
         ...
         IF (ppm_dim.EQ.3) THEN
            DO i=1,nnbox
               mincut_(1)=REAL(mincut(1,i),MK)
               mincut_(2)=REAL(mincut(2,i),MK)
               mincut_(3)=REAL(mincut(3,i),MK)

               maxcut_(1)=REAL(maxcut(1,i),MK)
               maxcut_(2)=REAL(maxcut(2,i),MK)
               maxcut_(3)=REAL(maxcut(3,i),MK)

               DO j=1,Np
                  ip = tree_lpdx(tree_lhbx(1,cutbox)+j-1)
                  IF ((xp(1,ip).GE.mincut_(1)).AND. &
                  &   (xp(1,ip).LT.maxcut_(1)).AND. &
                  &   (xp(2,ip).GE.mincut_(2)).AND. &
                  &   (xp(2,ip).LT.maxcut_(2)).AND. &
                  &   (xp(3,ip).GE.mincut_(3)).AND. &
                  &   (xp(3,ip).LT.maxcut_(3))) THEN
                     pbox(j) = i
                  ENDIF
               ENDDO
            ENDDO
