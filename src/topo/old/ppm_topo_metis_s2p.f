      ...
      !!! using the METIS grid partitioning library version 4.
      !!!
      !!! [NOTE]
      ...
      fail('PPM was compiled without Metis support',ppm_err_nometis,ppm_error=ppm_error_fatal)
#else
      ...
      !-------------------------------------------------------------------------
      FORALL (j=1:points,isub=1:nsubs) cornerno(j,isub) = 0
  ...
  !    NULLIFY(vsize)    ! we make no use of communication weights
      ...
      &    assig .EQ. ppm_param_assign_dual_cut) THEN
         ...
         ! partition.
         IF (ppm_debug .GT. 0) THEN
            stdout_f('(A,I6)',"METIS returned edgecut = ",edgecut)
         ENDIF
      ...
      &       assig .EQ. ppm_param_assign_dual_comm) THEN
         ...
         ! partition.
         IF (ppm_debug .GT. 0) THEN
            stdout_f('(A,I6)',"METIS returned communication volume = ",volume)
         ENDIF
      ELSE
         fail('Unknown METIS assignment scheme. Bailing out.')
      ENDIF
