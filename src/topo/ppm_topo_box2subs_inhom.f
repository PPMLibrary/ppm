      !-------------------------------------------------------------------------
      !  Subroutine   :                 ppm_topo_box2subs_inhom
      !-------------------------------------------------------------------------
      ...
      SUBROUTINE ppm_topo_box2subs_inhom_s(min_box,max_box,minboxsizes,nchld,nbox,   &
     &    min_sub,max_sub,minboxsizes_out,nsubs,info,boxid,level,blevel,child)
#elif __KIND == __DOUBLE_PRECISION
      SUBROUTINE ppm_topo_box2subs_inhom_d(min_box,max_box,minboxsizes,nchld,nbox,   &
     &    min_sub,max_sub,minboxsizes_out,nsubs,info,boxid,level,blevel,child)
      ...
      !!! minimum box sizes for all subs
      !!!
      !!! 1st index: x,y[,z]                                                   +
      !!! 2nd: box ID
      ...
      REAL(MK), DIMENSION(:,:), POINTER       :: minboxsizes_out
      !!! minimum box sizes for all subs (out for topo store)
      !!!
      !!! 1st index: x,y[,z]                                                   +
      !!! 2nd: box ID
      ...
      !  Initialise 
      !-------------------------------------------------------------------------
      CALL substart('ppm_topo_box2subs_inhom',t0,info)
      ...
      IF (info.NE.0) THEN
          ...
          CALL ppm_error(ppm_err_alloc,'ppm_topo_box2subs_inhom',          &
     ...
     &                            __LINE__,info)
              ...
              IF (info.NE.0) THEN
                  ...
                  CALL ppm_error(ppm_err_dealloc,'ppm_topo_box2subs_inhom',        &
     &                'stack of boxes to traverse BOXSTACK',__LINE__,info)
              ENDIF 
          ENDIF
      ELSE
     ...
     &        'maximum extent of subs MAX_SUB',__LINE__,info)
      ...
      CALL ppm_alloc(minboxsizes_out,ldc,iopt,info)
      IF (info.NE.0) THEN
          info = ppm_error_fatal
          CALL ppm_error(ppm_err_alloc,'ppm_topo_box2subs',          &
     &        'minimum extent of subs MINBOXSIZES_OUT',__LINE__,info)
          GOTO 9999
      ENDIF
     ...
     &            'box ID of subs BOXID',__LINE__,info)
      ...
      IF (ppm_dim .EQ. 2) THEN
          DO i=1,nsubs
              ...
              minboxsizes_out(1,i) = minboxsizes(1,j)
              minboxsizes_out(2,i) = minboxsizes(2,j)
          ENDDO
      ELSE
          DO i=1,nsubs
              j = subbox(i)
              ...
              minboxsizes_out(1,i) = minboxsizes(1,j)
              minboxsizes_out(2,i) = minboxsizes(2,j)
              minboxsizes_out(3,i) = minboxsizes(3,j)
          ENDDO
      ENDIF
     ...
     &        'list of box IDs SUBBOX',__LINE__,info)
      ...
      !-------------------------------------------------------------------------
 9999 CONTINUE
      CALL substop('ppm_topo_box2subs_inhom',t0,info)
      RETURN
      CONTAINS
      SUBROUTINE check
         IF (nbox .LT. 0) THEN
            info = ppm_error_error
            CALL ppm_error(ppm_err_argument,'ppm_topo_box2subs_inhom',     &
...
#if   __KIND == __SINGLE_PRECISION
      END SUBROUTINE ppm_topo_box2subs_inhom_s
#elif __KIND == __DOUBLE_PRECISION
      END SUBROUTINE ppm_topo_box2subs_inhom_d
#endif
