      ...
      SUBROUTINE ppm_topo_cost_s(xp,Np,min_sub,max_sub,nsubs,nnodes,cost_,info,pcost)
#elif __KIND == __DOUBLE_PRECISION
      SUBROUTINE ppm_topo_cost_d(xp,Np,min_sub,max_sub,nsubs,nnodes,cost_,info,pcost)
#endif
      ...
      REAL(MK), DIMENSION(:  ),           POINTER       :: cost_
      ...
      REAL(ppm_kind_double), DIMENSION(:), ALLOCATABLE :: cost
      REAL(ppm_kind_double)                            :: ll,kk
#ifdef __MPI
      REAL(ppm_kind_double), DIMENSION(:), ALLOCATABLE :: costsum
#endif
      REAL(MK),              DIMENSION(ppm_dim)        :: len_sub

      INTEGER, DIMENSION(ppm_dim)    :: ldu
      INTEGER                        :: i,ipart,j
      ...
      ldu(1) = nsubs
      CALL ppm_alloc(cost_,ldu,iopt,info)
      or_fail_alloc('costs per sub COST',ppm_error=ppm_error_fatal)

      ALLOCATE(cost(nsubs),STAT=info)
      or_fail_alloc('costs per sub COST',ppm_error=ppm_error_fatal)

      !-------------------------------------------------------------------------
      !  And initialize to zero
      !-------------------------------------------------------------------------
      cost = 0.0_ppm_kind_double
               ...
               IF (ppm_dim.GT.2) THEN
                  ...
                  &   xp(3,ipart).LT.max_sub(3,idom)) THEN
                     ...
                     IF (PRESENT(pcost)) THEN
                        cost(idom) = cost(idom) + REAL(pcost(ipart),ppm_kind_double)
                     ELSE
                        cost(idom) = cost(idom) + 1.0_ppm_kind_double
                     ENDIF
                  ELSE
                     !------------------------------------------------------
      ...
      IF (SIZE(nnodes,2).GE.nsubs) THEN
         ...
         IF (ppm_dim .EQ. 3) THEN
            DO i=1,nsubs
               cost(i) = cost(i) + REAL(nnodes(1,i),ppm_kind_double)* &
               &                   REAL(nnodes(2,i),ppm_kind_double)* &
               &                   REAL(nnodes(3,i),ppm_kind_double)
            ENDDO
         ELSE
            DO i=1,nsubs
               cost(i) = cost(i) + REAL(nnodes(1,i),ppm_kind_double)* &
      ...
      ELSE IF (Np.LT.1) THEN
         ...
         IF (ppm_dim .EQ. 3) THEN
            DO i=1,nsubs
               ...
               cost(i) = REAL(len_sub(1),ppm_kind_double)*REAL(len_sub(2),ppm_kind_double)*REAL(len_sub(3),ppm_kind_double)
            ENDDO
         ELSE
            DO i=1,nsubs
               len_sub(1) = max_sub(1,i)-min_sub(1,i)
               len_sub(2) = max_sub(2,i)-min_sub(2,i)
               cost(i) = REAL(len_sub(1),ppm_kind_double)*REAL(len_sub(2),ppm_kind_double)
            ENDDO
         ENDIF

         ll=MINVAL(cost)
         IF (ll.GT.0.0_ppm_kind_double) THEN
            kk=1.0_ppm_kind_double
            DO WHILE (ll*kk.LE.1.0_ppm_kind_double)
               kk=kk*10.0_ppm_kind_double
            ENDDO
            IF (kk.LT.10.0_ppm_kind_double**10) THEN
               cost=cost*kk
            ENDIF
         ENDIF
      ENDIF
      ...
      DEALLOCATE(cost,STAT=info)
      or_fail_dealloc("Failed to deallocate cost!")
