      ...
      &          Npart,Mpart,nvlist_cross,vlist_cross,info)
...
#if   __LDA == 1
        CHARACTER(LEN=ppm_char) :: caller = 'sop_approx_wp_1d'
#elif __LDA == 2
        ...
        particle_loop: DO ip = 1,Npart
           IF ( nvlist_cross(ip) .EQ. 0) THEN
              ...
              stdout("error in sop_approx_D.Cross lists should not be empty")
              info = -1
              GOTO 9999
           ENDIF
        ...
        IF (info .NE. 0) THEN
           stdout("ppm_map_part_ghost (push) failed.")
           info = -1
           GOTO 9999
        ENDIF
        CALL ppm_map_part_send(Npart,Mpart,info)
        IF (info .NE. 0) THEN
           stdout("ppm_map_part_ghost (send) failed.")
           info = -1
           GOTO 9999
        ENDIF
        CALL ppm_map_part_pop(wp,Npart,Mpart,info)
        IF (info .NE. 0) THEN
           stdout("ppm_map_part_ghost (pop) failed.")
           info = -1
           GOTO 9999
        ENDIF
#elif __LDA == 2
