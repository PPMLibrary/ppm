...
#if   __DIM  == __2D
      SUBROUTINE ppm_vtk_mesh_2d(filename,Mesh,info,step,Field)
#elif   __DIM  == __3D
      SUBROUTINE ppm_vtk_mesh_3d(filename,Mesh,info,step,Field)
#endif
...
!       CLASS(ppm_t_subpatch_),      POINTER :: p     => NULL()
      ...
      IF (PRESENT(Field)) THEN
         check_true(<#Field%is_discretized_on(Mesh)#>)
      ENDIF
...
#include "vtk/print_header.f"
         ...
         DO WHILE (ASSOCIATED(fld))
            ...
            CASE (ppm_type_real)
               IF (fld%lda.EQ.1) THEN
                  WRITE(iUnit,'(3A)') "      <PDataArray Name='", &
                  & TRIM(ADJUSTL(fld%name)),"' type='Float64' />"
               ELSE
                  DO icomp=1,fld%lda
                     WRITE(iUnit,'(3A,I0,A)') "      <PDataArray Name='", &
                     & TRIM(ADJUSTL(fld%name)),"_",icomp,"' type='Float64' />"
                  ENDDO
               ENDIF
            ...
            CASE (ppm_type_int)
               IF (fld%lda.EQ.1) THEN
                  WRITE(iUnit,'(3A)') "      <PDataArray Name='", &
                  & TRIM(ADJUSTL(fld%name)),"' type='Int32' />"
               ELSE
                  DO icomp=1,fld%lda
                     WRITE(iUnit,'(3A,I0,A)') "      <PDataArray Name='", &
                     & TRIM(ADJUSTL(fld%name)),"_",icomp,"' type='Int32' />"
                  ENDDO
               ENDIF

            END SELECT
            IF (PRESENT(Field)) EXIT
            fld => Mesh%mdata%next()
         ENDDO
...
#include "vtk/print_fields_header1.f"
               ...
               pdat1: DO WHILE (ASSOCIATED(pdat))
                  IF (PRESENT(Field)) THEN
                     ASSOCIATE (pdatd => pdat%discr_data)
                     IF (ASSOCIATED(pdatd%field_ptr,Field)) THEN
                     ...
                     ELSE
                        ...
                        CYCLE pdat1
                     ENDIF
                     END ASSOCIATE !(pdatd => pdat%discr_data)
                  ENDIF
                  IF (pdat%discr_data%lda.EQ.1) THEN
                     WRITE(iUnit,'(A)',ADVANCE='NO') &
                     & TRIM(ADJUSTL(pdat%discr_data%name))
         ...
         ! BACKSPACE(iUnit)
               ...
               pdat => p%subpatch_data%begin()
               pdat2: DO WHILE (ASSOCIATED(pdat))
                  IF (PRESENT(Field)) THEN
                     ASSOCIATE (pdatd => pdat%discr_data)
                     IF (ASSOCIATED(pdatd%field_ptr,Field)) THEN
...
#if   __DIM == __2D
      END SUBROUTINE ppm_vtk_mesh_2d
#elif   __DIM == __3D
      END SUBROUTINE ppm_vtk_mesh_3d
#endif
